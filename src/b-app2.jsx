          ...
          /** Lienzo para lÃ¡piz Ã³ptico / dedo: pointer capture, sin scroll al trazar. Herramientas: lÃ¡piz, goma, marcador, subrayado + deshacer. */
          const TabletWritingCanvas = ({ padKey, grid, strokeW, compareTarget, onOcrCompared, snapshotData, snapshotPadKey, onSnapshotChange, backgroundImageData = '' }) => {
              ...
              const runHandwritingOcr = async () => {
                  if (typeof Tesseract === 'undefined') {
                      setOcrErr('No se pudo cargar Tesseract.js. Comprueba la conexiÃ³n y recarga.');
                      return;
                  }
                  ...
                  setOcrLoading(true);
                  setOcrHint('Preparando imagenâ€¦');
                  let worker;
                  try {
                      const bw = canvasToBlackOnWhite(source);
                      worker = await Tesseract.createWorker('deu', 1, {
                          logger: (m) => {
                              if (m.status === 'recognizing text' && m.progress != null) setOcrHint(`Leyendoâ€¦ ${Math.round(100 * m.progress)}%`);
                      ...
                      if (!t) setOcrHint('No se detectÃ³ texto. Escribe mÃ¡s grande o con mÃ¡s contraste.');
                      else setOcrHint('');
                  ...
                  <div className="space-y-3">
                      <div className="rounded-xl border border-white/10 bg-black/25 p-2 md:p-3 space-y-2">
                          ...
                          <div className="flex flex-wrap gap-1.5 justify-center md:justify-start">
                              {[
                                  { id: 'pen', label: 'LÃ¡piz', icon: 'pen-line', title: 'Trazo normal (usa el grosor de abajo)' },
                                  ...
                                  { id: 'underline', label: 'Subrayado', icon: 'minus', title: 'LÃ­nea fina bajo el trazo (subrayar palabras)' },
                              ].map((t) => (
                                  <button
                                      key={t.id}
                                      type="button"
                          ...
                          {writingTool === 'eraser' && (
                              <div className="flex flex-wrap items-center gap-2 justify-center md:justify-start pt-1 border-t border-white/5">
                                  ...
                                  <span className="text-[9px] text-slate-600 ml-1">Fino = letra Â· ancho = palabra</span>
                              </div>
                          )}
                          ...
                          {writingTool === 'underline' && (
                              <p className="text-[9px] text-sky-400/90 text-center md:text-left pt-1 border-t border-white/5">Subrayado: traza encima de la lÃ­nea; la raya azul sale un poco mÃ¡s abajo (como subrayar en papel).</p>
                          )}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 justify-center items-center">
                          <button type="button" onClick={undoLastStroke} disabled={!canUndo} className="px-4 py-2 rounded-lg bg-slate-600 hover:bg-slate-500 disabled:opacity-35 disabled:pointer-events-none text-sm font-bold" title="Deshacer el Ãºltimo trazo">
                              <span className="inline-flex items-center gap-1.5"><Icon name="undo-2" className="w-4 h-4" /> Deshacer trazo</span>
                          </button>
                          <button type="button" onClick={clearPad} className="px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-sm font-bold" title="VacÃ­a todo el lienzo">Borrar lienzo</button>
                          ...
                          <button
                              ...
                              title="OCR gratuito en tu dispositivo (Tesseract). La primera vez descarga el idioma alemÃ¡n."
                          >
                              {ocrLoading ? <span className="animate-pulse">â³ OCRâ€¦</span> : <><Icon name="scan-line" className="w-4 h-4" /> Reconocer texto</>}
                          </button>
                      </div>
                      <p className="text-[10px] text-center text-slate-500 px-1">
                          Motor: <strong className="text-slate-400">Tesseract.js</strong> (alemÃ¡n, local). El manuscrito es aproximado; la letra muy ligada o muy pequeÃ±a empeora el resultado.
                      </p>
                      {ocrComparePct !== null && (
              ...
              if (Array.isArray(savedScripts)) {
                  savedScripts.forEach((s) => {
                      try {
                          ...
                          out.push({ id: String(s.id), title: s.title || 'Sin tÃ­tulo', count });
                      } catch (e) {
                          out.push({ id: String(s.id), title: s.title || 'Sin tÃ­tulo', count: 0 });
                      }
                  });
              }
              return out;
          ...
          const getSelfCheckItems = useCallback(() => {
              const items = [
                  { id: 'mic-api', label: 'MicrÃ³fono API', ok: !!(navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === 'function') },
                  ...
                  { id: 'telc-input', label: 'TELC teclado/lÃ¡piz', ok: writingMode !== 'telc' || ['pen', 'keyboard'].includes(writingTelcInputMode) }
              ];
              return items;
          ...
          const runTelcWritingCoach = useCallback((rawText, task) => {
              ...
              const suggestion = [
                  scoreTask < 4 ? 'AÃ±ade mÃ¡s contenido concreto del encargo (datos, peticiÃ³n y cierre).' : null,
                  ...
                  scoreGrammar < 4 ? 'Revisa signos de puntuaciÃ³n, mayÃºsculas de sustantivos y umlauts.' : null
              ].filter(Boolean);
              setWritingTelcCoach({
                  total,
                  max,
                  ...
                  scoreCohesion,
                  scoreGrammar,
                  suggestionText: suggestion.length ? suggestion.join(' ') : 'Muy buen texto para TELC. Solo pule estilo y precisiÃ³n.'
              });
          }, []);
          ...
          const activeSectionMeta = useMemo(() => {
              const map = {
                  inicio: { icon: 'layout-dashboard', title: 'Inicio', desc: 'Panel rÃ¡pido y continuidad', tone: 'border-indigo-500/35 bg-indigo-900/25 text-indigo-200' },
                  ruta: { icon: 'map', title: 'Ruta', desc: 'Camino A1-A2 guiado', tone: 'border-fuchsia-500/35 bg-fuchsia-900/25 text-fuchsia-200' },
                  historia: { icon: 'play', title: 'Historia', desc: 'Escucha, modos y simulaciÃ³n oral', tone: 'border-blue-500/35 bg-blue-900/25 text-blue-200' },
                  lectura: { icon: 'mic', title: 'Lectura', desc: 'Leer en voz alta y comparar', tone: 'border-sky-500/35 bg-sky-900/25 text-sky-200' },
                  shadowing: { icon: 'audio-lines', title: 'Shadowing', desc: 'Fluidez y pronunciaciÃ³n', tone: 'border-teal-500/35 bg-teal-900/25 text-teal-200' },
                  ...
                  entrenamiento: { icon: 'graduation-cap', title: 'Entrenamiento', desc: 'ArtÃ­culos, prep. y simulacro', tone: 'border-fuchsia-500/35 bg-fuchsia-900/25 text-fuchsia-200' },
                  ...
                  progreso: { icon: 'bar-chart', title: 'Progreso', desc: 'MÃ©tricas, export y objetivos', tone: 'border-yellow-500/35 bg-yellow-900/25 text-yellow-200' },
                  guiones: { icon: 'file-text', title: 'Biblioteca', desc: 'Guiones y distribuciÃ³n', tone: 'border-purple-500/35 bg-purple-900/25 text-purple-200' },
                  lexikon: { icon: 'library', title: 'Lexikon', desc: 'Diccionario y traducciÃ³n', tone: 'border-cyan-500/35 bg-cyan-900/25 text-cyan-200' },
                  telc: { icon: 'clipboard-check', title: 'TELC', desc: 'GuÃ­a por niveles A1-C2', tone: 'border-orange-500/35 bg-orange-900/25 text-orange-200' },
                  storybuilder: { icon: 'sparkles', title: 'IA', desc: 'GeneraciÃ³n y resumen de contenido', tone: 'border-fuchsia-500/35 bg-fuchsia-900/25 text-fuchsia-200' },
                  historiaspro: { icon: 'feather', title: 'Historias Pro', desc: 'ES/DE/OCR y estilizaciÃ³n', tone: 'border-emerald-500/35 bg-emerald-900/25 text-emerald-200' },
                  comunidad: { icon: 'trophy', title: 'Comunidad', desc: 'EconomÃ­a, directorio y liga', tone: 'border-violet-500/35 bg-violet-900/25 text-violet-200' },
              };
              return map[activeTab] || { icon: 'layout-dashboard', title: 'Inicio', desc: 'Panel rÃ¡pido y continuidad', tone: 'border-indigo-500/35 bg-indigo-900/25 text-indigo-200' };
          ...
          const activeModeBadge = useMemo(() => {
              ...
              if (activeTab === 'entrenamiento') return 'Simulacro y prÃ¡ctica avanzada';
          ...
          const activeSectionHint = useMemo(() => {
              const h = {
                  inicio: 'Sugerencia: abre Entrenamiento si quieres prÃ¡ctica tipo examen.',
                  ruta: 'Sugerencia: completa lecciÃ³n + huecos + voz para cerrar ciclo.',
                  ...
                  vocabulario: 'Sugerencia: marca FÃ¡cil solo si recuerdas sin mirar traducciÃ³n.',
                  entrenamiento: 'Sugerencia: alterna categorÃ­a dÃ©bil y simulacro corto.',
                  ...
                  lexikon: 'Sugerencia: guarda pares Ãºtiles y repÃ¡salos en vocabulario.',
                  ...
                  historiaspro: 'Sugerencia: genera en A2/B1 y envÃ­a escenas a Historia.',
                  comunidad: 'Sugerencia: revisa economÃ­a y liga al final de cada sesiÃ³n.',
              };
              return h[activeTab] || '';
          }, [activeTab]);
          ...
          return (
            <div className={`flex flex-col muller-main-fill h-[100svh] max-h-[100svh] w-full font-sans md:rounded-xl overflow-x-hidden md:overflow-hidden shadow-2xl relative transition-colors duration-500 muller-theme-bg ${reduceMotionUi ? 'muller-reduce-motion' : ''} ${themeShellClass} ${uiTheme === 'light' ? 'text-slate-900' : 'text-white'} ${getBgColor()}`}>
              ...
              {showSplash && (
                  <div className="fixed inset-0 z-[300] overflow-hidden" role="dialog" aria-modal="true" aria-label="Pantalla de inicio Profesor Plaza MÃ¼ller">
                      <div className="absolute inset-0 flex flex-col">
                          ...
                          <div className="h-[160px] md:h-[192px] bg-white border-y-2 border-black/20 shadow-[0_8px_25px_rgba(0,0,0,0.25)] flex items-stretch px-2 md:px-4">
                              <button
                                  ...
                                  aria-label="Entrar a la aplicaciÃ³n pulsando el logo"
                                  title="Pulsa el logo para entrar"
                              >
                                  <img
                                      src="./icons/profesor-plaza-muller-logo.jpg"
                                      alt="Logo Profesor Plaza MÃ¼ller"
              ...
              <div className="muller-top-nav fixed top-0 left-0 right-0 w-full bg-gradient-to-b from-zinc-900/95 via-black/90 to-black/95 backdrop-blur-xl border-b border-white/[0.07] shadow-[0_4px_24px_rgba(0,0,0,0.35)] p-1.5 md:p-2.5 pt-[max(0.35rem,env(safe-area-inset-top,0px))] flex flex-col gap-1.5 justify-stretch items-stretch z-20">
                  ...
                  <div className="flex flex-shrink-0 items-center gap-1.5">
                      <button type="button" onClick={() => window.__MULLER_OPEN_EXERCISE_HELP && window.__MULLER_OPEN_EXERCISE_HELP('nav_inicio')} className="text-[10px] font-black text-amber-200/90 hover:text-white underline underline-offset-2 px-2 py-1 rounded-lg border border-white/10 hover:bg-white/10" title="Ayuda contextual rÃ¡pida">
                          Ayuda
                      </button>
                      <span className="inline-flex flex-col items-center gap-0.5">
                          <button type="button" aria-label="Centro MÃ¼ller: voces, plan del dÃ­a y ayuda" onClick={() => { setShowMullerHub(true); setMullerHubTab('voices'); }} className="bg-gradient-to-br from-sky-600 to-indigo-900 hover:from-sky-500 hover:to-indigo-800 text-white p-2 rounded-full shadow-lg ring-2 ring-white/15 transition" title="Voces del sistema, plan del dÃ­a, retos y tour">
                          ...
                          <div className="relative flex items-center mr-1 md:mr-2 border-r border-white/20 pr-2 md:pr-3" ref={userMenuWrapRef}>
                              ...
                              {userMenuOpen && (
                                  ...
                                  >
                                      ...
                                      >
                                          <Icon name="upload" className="h-4 w-4 shrink-0 text-indigo-400" /> Importar datosâ€¦
                                      </button>
                                      <button
                                          type="button"
                                          role="menuitem"
              ...
              <div className="w-full border-b border-white/[0.08] bg-black/35 backdrop-blur-md px-3 md:px-4 py-2 md:py-2.5 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                          <p className="text-[11px] font-black uppercase tracking-wider text-cyan-300 truncate">MÃ¼ller Â· secciÃ³n activa</p>
                          <p className="text-sm font-bold text-white truncate">{activeSectionMeta.title} <span className="text-gray-400 font-semibold">Â· {activeSectionMeta.desc}</span></p>
                  ...
                  <div className="flex items-center gap-2 flex-wrap md:justify-end">
                      ...
                      <span className="px-2.5 py-1 rounded-full text-[11px] font-bold border border-rose-500/35 bg-rose-950/40 text-rose-200">â¤ï¸ {userStats.hearts}</span>
                          ...
                          <div className="p-4 md:p-5 overflow-y-auto max-h-[calc(90vh-8.5rem)]">
                              {profileSettingsTab === 'perfil' && (
                                  <div className="space-y-4">
                                      <div className="rounded-xl border border-white/10 bg-black/25 p-4 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                                          ...
                                          <div><p className="text-gray-500">Racha</p><p className="font-black text-orange-300">{userStats.streakDays || 0} dÃ­as</p></div>
                                      </div>
                                      {unifiedAuth ? (
                                          <div className="space-y-3">
                                              <p className="text-sm text-emerald-300 font-bold">SesiÃ³n iniciada Â· {unifiedAuth.source === 'supabase' ? 'Supabase' : 'Local'}</p>
                                                  ...
                                                  <div className="flex flex-col sm:flex-row gap-2">
                                                      ...
                                                      <button type="button" disabled={profileNameBusy} className="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-50 font-bold text-sm"
                                                          onClick={async () => {
                                                              ...
                                                              if (!nextName) { setProfileNameMsg('Escribe un nombre vÃ¡lido.'); return; }
                                                          ...
                                                          }}>{profileNameBusy ? 'Guardandoâ€¦' : 'Guardar nombre'}</button>
                                                  </div>
                                          ...
                                          <div className="space-y-3">
                                              ...
                                              <input type="password" autoComplete={authMode === 'register' ? 'new-password' : 'current-password'} className="w-full bg-black/50 border border-white/15 rounded-xl px-3 py-2.5 text-white outline-none focus:border-violet-500" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} placeholder="ContraseÃ±a (mÃ­n. 6)" />
                                              ...
                                              <button type="button" disabled={authBusy} className="w-full py-3 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-50 font-black text-white"
                                                  onClick={async () => {
                                                      ...
                                                      const errMap = { CRYPTO_UNAVAILABLE: 'Necesitas https o localhost para registrar con cifrado seguro.', EMAIL_INVALID: 'Introduce un email vÃ¡lido.', PASS_SHORT: 'La contraseÃ±a debe tener al menos 6 caracteres.', EMAIL_TAKEN: 'Ese email ya estÃ¡ registrado en este dispositivo.', BAD_CREDENTIALS: 'Email o contraseÃ±a incorrectos.' };
                                                      try {
                                                          const client = mullerGetSupabaseClient();
                                                          if (client) {
                                                  ...
                                                  }}>{authBusy ? 'â€¦' : authMode === 'register' ? 'Crear cuenta' : 'Entrar'}</button>
                                          </div>
                                      )}
                                  </div>
                              )}
                              {profileSettingsTab === 'ajustes' && (
                                  <div className="space-y-4">
                                      ...
                                      <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                                          ...
                                          <div className="flex flex-wrap gap-2">
                                              <button type="button" onClick={() => setShowFloatingTools((v) => !v)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${showFloatingTools ? 'bg-cyan-700 border-cyan-400 text-white' : 'bg-slate-800 border-white/10 text-gray-300'}`}>Herramientas rÃ¡pidas (Ajustes): {showFloatingTools ? 'ON' : 'OFF'}</button>
                                              ...
                                              <button type="button" onClick={() => { setShowMullerHub(true); setMullerHubTab('voices'); setShowProfileSettingsModal(false); }} className="px-3 py-1.5 rounded-lg text-xs font-bold border border-sky-500/30 bg-sky-900/30 text-sky-200">MÃ¡s ajustes de vocesâ€¦</button>
                                          </div>
                                      </div>
                                      <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                                          <p className="text-xs font-bold uppercase tracking-widest text-indigo-300 mb-2">Respaldo y sincronizaciÃ³n</p>
                                          <div className="flex flex-wrap gap-2">
                                              ...
                                              <button type="button" onClick={() => window.dispatchEvent(new Event('muller-show-sync-help'))} className="px-3 py-1.5 rounded-lg text-xs font-bold border border-teal-500/35 bg-teal-900/35 text-teal-200">GuÃ­a de sincronizaciÃ³n</button>
                                              <button type="button" onClick={() => window.dispatchEvent(new Event('muller-request-mic'))} className="px-3 py-1.5 rounded-lg text-xs font-bold border border-green-500/35 bg-green-900/35 text-green-200">Permiso de micrÃ³fono</button>
                                          </div>
                                      </div>
                                  </div>
                              )}
                              {profileSettingsTab === 'atajos' && (
                                  <div className="space-y-2 text-sm text-gray-300">
                                      <p><kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">?</kbd> Ayuda rÃ¡pida</p>
                                      <p><kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">I</kbd> Inicio Â· <kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">R</kbd> Ruta Â· <kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">H</kbd> Historia</p>
                                      <p><kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">V</kbd> Vocab Â· <kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">P</kbd> Progreso Â· <kbd className="px-1.5 py-0.5 rounded bg-black/50 border border-white/20 text-xs">M</kbd> Centro MÃ¼ller</p>
              ...
              {showDeathModal && (
                  <div className="absolute inset-0 z-50 bg-black/95 flex flex-col justify-center items-center p-4 md:p-8 animate-in zoom-in duration-300">
                      ...
                      <h1 className="text-3xl md:text-5xl font-black text-white mb-4 text-center">Â¡Te has quedado sin vidas!</h1>
                      <div className="flex gap-4">
                          <button onClick={buyHearts} className="bg-yellow-500 hover:bg-yellow-400 text-black px-6 md:px-8 py-3 md:py-4 rounded-xl font-black text-base md:text-xl shadow-xl flex items-center gap-2"><Icon name="coins" className="w-5 h-5 md:w-6 md:h-6" /> Comprar 5 â¤ï¸ (50 Coins)</button>
                      </div>
                  </div>
              )}
              ...
              {showOnboarding && onboardingStep >= 1 && onboardingStep <= 5 && (() => {
                  const obSteps = [
                      { t: 'Bienvenida', d: 'MÃ¼ller funciona en el navegador: Historia (audio), Vocab con SRS, Escritura con OCR local, B1/B2 y Progreso. Todo gratis en este dispositivo.' },
                      { t: 'PestaÃ±as', d: 'Arriba cambias de actividad. La pestaÃ±a Entrenamiento abre artÃ­culos, verbos y preposiciones con simulacro. El panel azul es el Centro MÃ¼ller (voces, plan, ayuda).' },
                      ...
                      { t: 'Objetivos y racha', d: 'En Vocab configuras objetivo diario de tarjetas; la racha solo sube si hay actividad mÃ­nima real (umbrales fijos en el informe del Centro).' },
                  ...
                  <div className="fixed inset-0 z-[128] bg-black/85 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => finishOnboarding()} role="presentation">
                      <div className="bg-slate-900 border border-sky-500/50 rounded-2xl p-6 max-w-md w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
                          <p className="text-[10px] font-bold text-sky-400 uppercase tracking-widest mb-2">Onboarding Â· paso {onboardingStep}/5</p>
                      ...
                      <div className="bg-slate-900 border border-purple-600/50 p-4 md:p-6 rounded-2xl max-w-md w-full shadow-2xl max-h-[85vh] overflow-y-auto">
                          ...
                          <div className="flex gap-2">
                              ...
                              <button onClick={() => {
                                  ...
                                  if (selected.length === 0) { alert("Selecciona al menos una lecciÃ³n."); return; }
                                  let words = [];
              ...
              {showMullerHub && (
                  <div className="fixed inset-0 z-[85] bg-black/90 backdrop-blur-md flex items-center justify-center p-3 md:p-6" onClick={() => setShowMullerHub(false)} role="presentation">
                      <div role="dialog" aria-modal="true" aria-labelledby="muller-hub-title" className="bg-slate-900 border border-sky-500/40 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-hidden shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
                          <h2 id="muller-hub-title" className="sr-only">Centro MÃ¼ller</h2>
                          ...
                          <div className="p-4 md:p-5 overflow-y-auto text-sm">
                              {mullerHubTab === 'voices' && (
                                  <div className="space-y-4">
                                      <p className="text-xs text-gray-400">Las voces <strong className="text-white">neural / premium</strong> (si las trae tu navegador) suelen sonar mÃ¡s naturales. Todo es <strong className="text-white">gratis</strong>: usa el motor de voz del sistema (Chrome/Edge suelen traer mÃ¡s opciones).</p>
                                      <button
                                          type="button"
                                      ...
                                      <div>
                                          <label className="text-[10px] font-bold text-sky-300 uppercase tracking-wider">AlemÃ¡n (Historias, Shadowing, B1/B2â€¦)</label>
                                          <select
                                          ...
                                          >
                                              <option value="">Predeterminada (automÃ¡tica MÃ¼ller)</option>
                                          ...
                                          <button
                                              ...
                                              onClick={() => {
                                                  const u = new SpeechSynthesisUtterance('Guten Tag, ich Ã¼be Deutsch mit Professor MÃ¼ller.');
                                                  u.lang = 'de-DE';
                                      ...
                                      <div>
                                          <label className="text-[10px] font-bold text-amber-300 uppercase tracking-wider">EspaÃ±ol (traducciones en podcast / vocab)</label>
                                          <select
                                              className="mt-1 w-full bg-black/50 border border-white/15 rounded-lg p-2 text-white text-xs"
                                              value={ttsEsUri}
                                              onChange={(e) => {
                                          ...
                                          >
                                              <option value="">Predeterminada (automÃ¡tica)</option>
                                              {sortedEsVoices.map((v) => {
                                                  const uri = v.voiceURI || v.name;
                                                  return (
                                                  <option key={uri + v.name} value={uri}>{v.name} Â· {v.lang}</option>
                                                  );
                                              })}
                                          </select>
                                          <button
                              ...
                              {mullerHubTab === 'tips' && (
                                  <div className="space-y-3 text-xs text-gray-300">
                                      ...
                                      <p>Usa <strong className="text-white">Chrome o Edge</strong> para mÃ¡s voces y mejor reconocimiento de voz.</p>
                                      <p><strong className="text-white">GitHub Pages</strong> requiere HTTPS; el micrÃ³fono funciona al permitir permisos.</p>
                                      <p>Si la lista sale vacÃ­a, pulsa <strong className="text-white">Recargar lista de voces</strong> en la pestaÃ±a Voces o recarga la pÃ¡gina.</p>
                              ...
                              {mullerHubTab === 'chromeai' && (
                                  <div className="space-y-3 text-xs text-gray-300">
                                      ...
                                      <p className="text-[10px] text-gray-500">
                                          Requisitos tÃ­picos: Chrome de escritorio reciente, espacio libre en disco, modelo descargable desde Chrome. DocumentaciÃ³n:{' '}
              ...
              {tourStep >= 1 && tourStep <= 5 && (() => {
                  const steps = [
                      { t: 'Bienvenido al Entrenador MÃ¼ller', d: 'Historia es el centro: escenas, audio y modos (Diktat, quizâ€¦). Usa las pestaÃ±as arriba para cambiar de actividad.' },
                      ...
                      { t: 'Vocab y Progreso', d: 'FÃ¡cil/Normal/DifÃ­cil programa repaso espaciado (SRS). En Progreso ves mazos y exportas a Anki.' },
                      { t: 'Entrenamiento avanzado', d: 'Entrenamiento concentra artÃ­culos, verbos y preposiciones con seguimiento de precisiÃ³n.' },
                      { t: 'Shadowing, Escritura y B1/B2', d: 'Shadowing entrena pronunciaciÃ³n; Escritura incluye OCR; B1/B2 son frases modelo. Â¡Viel Erfolg!' },
                  ];
              ...
              {exerciseHelpId && MULLER_EXERCISE_HELP[exerciseHelpId] && (
                  <div className="fixed inset-0 z-[125] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setExerciseHelpId(null)} role="presentation">
                      <div role="dialog" aria-modal="true" aria-labelledby="exercise-help-title" className="bg-slate-900 border border-sky-500/40 rounded-2xl max-w-md w-full max-h-[85vh] overflow-y-auto shadow-2xl p-5 md:p-6" onClick={(e) => e.stopPropagation()}>
                          ...
                          <p className="text-[10px] text-gray-600 mt-3 text-center"><kbd className="px-1 py-0.5 rounded bg-black/50 border border-white/20">Esc</kbd> tambiÃ©n cierra</p>
                      </div>
                  </div>
              )}
              ...
              <div className={`muller-app-main flex-1 overflow-y-auto relative flex flex-col hide-scrollbar pt-[var(--muller-mobile-header-h)] pb-[calc(var(--muller-mobile-bottom-nav-h)+max(0.5rem,env(safe-area-inset-bottom,0px)))] ${activeTab === 'historia' && mode !== 'quiz' && mode !== 'interview' && !practiceActive ? 'muller-main-historia-pb' : ''} ${uiTheme === 'light' ? 'text-slate-900' : ''}`}>
                  ...
                  {activeTab === 'telc' && !practiceActive && (() => {
                      ...
                      <div className={`p-4 md:p-8 max-w-3xl mx-auto w-full animate-in fade-in duration-500 overflow-y-auto pb-24 ${uiTheme === 'light' ? 'text-slate-900' : ''}`}>
                          <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
                              <h1 className={`text-2xl md:text-4xl font-black flex items-center gap-2 ${uiTheme === 'light' ? 'text-orange-800' : 'text-orange-300'}`}><Icon name="clipboard-check" className="w-8 h-8 md:w-10 md:h-10" /> TELC Â· GuÃ­a y examen</h1>
                              <ExerciseHelpBtn helpId="nav_telc" />
                          </div>
                          <p className={`text-sm mb-4 leading-relaxed ${tcMuted}`}>OrientaciÃ³n educativa por nivel (CEFR). No es un modelo oficial de examen: para convocatorias y modelos actuales usa <a href="https://www.telc.net" target="_blank" rel="noopener noreferrer" className={`font-semibold hover:underline ${uiTheme === 'light' ? 'text-orange-700' : 'text-orange-400'}`}>telc.net</a> y tu centro.</p>
                          ...
                          <h2 className={`text-lg font-black mb-3 ${tcHeading}`}>DÃ­a del examen (todos los niveles)</h2>
                          <div className={`space-y-6 text-sm ${uiTheme === 'light' ? 'text-slate-700' : 'text-gray-300'}`}>
                              <section className={`rounded-2xl border p-4 md:p-6 ${tc}`}>
                                  <h3 className={`text-lg font-bold mb-2 flex items-center gap-2 ${tcHeading}`}><Icon name="briefcase" className="w-5 h-5" /> QuÃ© llevar</h3>
                                  <ul className="list-disc list-inside space-y-1.5">
                                      <li>Documento de identidad vÃ¡lido (mismo que al registrarte).</li>
                                      <li>ConfirmaciÃ³n / hoja de inscripciÃ³n al examen (si el centro la enviÃ³).</li>
                                      <li>LÃ¡piz negro o azul y goma (si el centro permite escritura a mano).</li>
                                      <li>Reloj analÃ³gico silencioso si te ayuda (sin smartwatch en sala).</li>
                                      <li>Llegada con margen: localiza baÃ±os y aula el dÃ­a anterior si puedes.</li>
                                  </ul>
                              </section>
                              <section className={`rounded-2xl border p-4 md:p-6 ${tc}`}>
                                  ...
                                  <ul className="list-disc list-inside space-y-1.5">
                                      ...
                                      <li>Oral: suele haber preparaciÃ³n corta; usa notas solo si el formato lo permite.</li>
                              ...
                              <section className={`rounded-2xl border p-4 md:p-6 ${tc}`}>
                                  ...
                                  <ul className="list-disc list-inside space-y-1.5">
                                      ...
                                      <li>Si no entiendes una instrucciÃ³n, pide aclaraciÃ³n corta en alemÃ¡n.</li>
                                      <li>En la redacciÃ³n: coherencia y conectores antes que vocabulario raro.</li>
                                  </ul>
                              </section>
                          </div>
                          <div className={`rounded-xl border border-dashed border-orange-500/35 p-4 mb-6 text-xs ${uiTheme === 'light' ? 'bg-orange-50 text-slate-600' : 'bg-orange-950/20 text-gray-400'}`}>
                              PrÃ¡ctica tipo test: abre la pestaÃ±a <strong className={uiTheme === 'light' ? 'text-fuchsia-700' : 'text-fuchsia-300'}>Entrenamiento</strong> (artÃ­culos, verbos+prep., preposiciones) y el simulacro con cronÃ³metro si estÃ¡ disponible en tu versiÃ³n.
                          </div>
                          <div className="flex flex-wrap gap-3 mt-2">
                              <button type="button" onClick={() => {
                                  const lines = [
                                      ...
                                      telcPack.summary,
                                      '',
                                      'QuÃ© llevar: DNI/documento, confirmaciÃ³n, lÃ¡pices, llegada temprana.',
                                      'Tiempos: gestionar lectura; planificar escritura; oral con preparaciÃ³n; escucha Ãºnica.',
                                      'Consejos: descanso, pedir aclaraciones, coherencia en la redacciÃ³n.',
                                  ];
                              ...
                              <button type="button" onClick={() => {
                                  ...
                                  if (!w) return;
                                  w.document.write(`<html><head><title>TELC MÃ¼ller ${telcLevel}</title></head><body style="font-family:sans-serif;padding:24px;max-width:640px;"><h1>TELC Â· ${telcLevel}</h1><p>${telcPack.summary}</p><p>Usa Ctrl+P para PDF.</p><h2>Checklist</h2><ul><li>Documento</li><li>ConfirmaciÃ³n</li><li>Material de escritura</li></ul></body></html>`);
                                  w.document.close();
                                  w.print();
                          ...
                          {!isGeneratingStory ? (
                              <div className="bg-fuchsia-900/40 p-4 md:p-8 rounded-2xl md:rounded-3xl border-2 border-fuchsia-500/50 shadow-2xl flex flex-col gap-4 md:gap-6">
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                      ...
                                      <div>
                                          <label className="text-fuchsia-300 font-bold mb-2 block uppercase tracking-widest text-xs md:text-sm">TemÃ¡tica:</label>
                                          <select className="w-full bg-black/50 border border-fuchsia-800 p-2 md:p-3 rounded-xl text-white outline-none focus:border-fuchsia-400 text-sm md:text-base" value={aiTheme} onChange={(e)=>setAiTheme(e.target.value)}>
                                              ...
                                              <option value="Krimi">PolicÃ­aca (Krimi)</option>
                                              ...
                                              <option value="SciFi">Ciencia FicciÃ³n (SciFi)</option>
                                              <option value="Romantik">Romance (Romantik)</option>
                                              <option value="Horror">Terror (Horror)</option>
                                              <option value="Geschichte">HistÃ³rico (Geschichte)</option>
                                              <option value="KomÃ¶die">Comedia (KomÃ¶die)</option>
                                          </select>
                                      </div>
                                  </div>
                                  <div>
                              ...
                              <div className="flex flex-col items-center justify-center h-48 md:h-64 gap-4 md:gap-6 animate-pulse">
                                  ...
                                  <h2 className="text-lg md:text-2xl font-bold text-fuchsia-300 text-center">La IA del Profesor MÃ¼ller estÃ¡ escribiendo un guion...</h2>
                              </div>
                          )}
                      </div>
                  )}
                      ...
                      <div className="p-4 md:p-8 max-w-5xl mx-auto w-full animate-in fade-in duration-500">
                          <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                              <div>
                                  ...
                                  <p className="text-sm text-emerald-100/80 mt-1">Escribe en espaÃ±ol/alemÃ¡n o sube manuscrito (OCR), y obtÃ©n versiÃ³n natural por nivel, simplificada, glosario y escenas para `Historia`.</p>
                              </div>
                              <div className={`text-xs px-3 py-2 rounded-xl border ${cloudSyncState === 'synced' ? 'bg-emerald-900/40 border-emerald-500/35 text-emerald-200' : cloudSyncState === 'syncing' ? 'bg-sky-900/35 border-sky-500/35 text-sky-200' : cloudSyncState === 'error' ? 'bg-rose-900/40 border-rose-500/40 text-rose-200' : 'bg-black/30 border-white/10 text-gray-300'}`}>
                                  Sync: {cloudSyncLabel}{cloudSyncAt ? ` Â· ${new Date(cloudSyncAt).toLocaleTimeString()}` : ''}
                              </div>
                          </div>
                          ...
                          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                              <section className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 md:p-5 space-y-3">
                                  <div className="flex flex-wrap gap-2">
                                      {[
                                          { id: 'es', label: 'Entrada espaÃ±ol' },
                                          { id: 'de', label: 'Entrada alemÃ¡n (corregir)' },
                                  ...
                                  <textarea value={storiesProSourceText} onChange={(e) => setStoriesProSourceText(e.target.value)} className="w-full h-52 bg-black/45 border border-white/15 rounded-xl p-3 text-white outline-none focus:border-emerald-500 resize-y" placeholder={storiesProInputMode === 'de' ? 'Escribe tu historia en alemÃ¡n para correcciÃ³n/estilizaciÃ³nâ€¦' : 'Escribe tu historia en espaÃ±olâ€¦'} />
                                  {storiesProErr ? <p className="text-xs text-rose-300">{storiesProErr}</p> : null}
                                  <div className="flex flex-wrap gap-2">
                                      <button type="button" disabled={storiesProBusy} onClick={runHistoriasProGenerate} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 font-bold text-sm">{storiesProBusy ? 'Procesandoâ€¦' : 'Generar Historias Pro'}</button>
                              ...
                              <section className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 md:p-5 space-y-3">
                                  {!storiesProResult ? (
                                      <p className="text-sm text-gray-400">Genera una historia para ver salida natural, versiÃ³n simplificada, glosario y escenas prÃ¡cticas.</p>
                                  ) : (
                                      <>
                                          <div className="rounded-xl border border-emerald-500/30 bg-emerald-950/25 p-3">
                                              <p className="text-[11px] uppercase tracking-wider font-bold text-emerald-300 mb-1">AlemÃ¡n natural ({storiesProLevel})</p>
                                          ...
                                          <div className="rounded-xl border border-sky-500/30 bg-sky-950/20 p-3">
                                              <p className="text-[11px] uppercase tracking-wider font-bold text-sky-300 mb-1">VersiÃ³n simplificada</p>
                                          ...
                                          <div className="rounded-xl border border-violet-500/30 bg-violet-950/20 p-3">
                                              <p className="text-[11px] uppercase tracking-wider font-bold text-violet-300 mb-1">Glosario rÃ¡pido</p>
                                              <div className="flex flex-wrap gap-2">
                  ...
                  {practiceActive && currentPracticeItem && (
                      <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 animate-in zoom-in">
                         ...
                         <div className="flex flex-wrap items-center justify-center gap-2 mb-6 md:mb-8">
                             <h2 className="text-2xl md:text-3xl font-black text-amber-500 uppercase tracking-widest"><Icon name="brain" className="w-6 h-6 md:w-8 md:h-8 inline mr-2" /> Entrenamiento RÃ¡pido</h2>
                      ...
                      <div className="flex-1 overflow-y-auto hide-scrollbar p-4 md:p-8 max-w-6xl mx-auto w-full animate-in fade-in duration-500">
                          <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
                              <div>
                                  ...
                                  <p className="text-xs md:text-sm text-fuchsia-100/80 mt-1">Centro principal de prÃ¡ctica: artÃ­culos, verbos con preposiciÃ³n, preposiciones y simulacro TELC.</p>
                              </div>
                          ...
                          <div className="flex flex-col md:flex-row md:flex-wrap justify-between items-start md:items-center mb-6 md:mb-8 border-b border-sky-900/50 pb-4 md:pb-6 gap-4">
                              <div className="min-w-0 flex-1 md:max-w-[min(100%,28rem)]">
                                  <h1 className={`text-2xl md:text-4xl font-black flex items-center gap-2 md:gap-3 ${bxBankLevel === 'b1' ? 'text-emerald-400' : 'text-sky-400'}`}>
                                      ...
                                      {bxBankLevel === 'b1' ? 'Banco B1 Â· Fundamentos' : 'Banco B2 Â· Meisterklasse'}
                                  </h1>
                              ...
                              <div className="flex flex-wrap items-center gap-2 justify-end">
                                  ...
                                  <ExerciseHelpBtn helpId={bxExerciseHelpId} title={'Ayuda: categorÃ­a ' + bxCategory} />
                              </div>
                              <div className="w-full md:basis-full flex flex-wrap items-center gap-2 mt-1 md:mt-0 pt-3 border-t border-white/10 md:order-last">
                                  ...
                                  {bxCategory !== 'mix' ? (
                                      ...
                                      <span className="text-[9px] text-gray-500">Elige una categorÃ­a (no MIX) para borrar solo esa subpestaÃ±a.</span>
                                  )}
                              </div>
                          </div>
                              ...
                              {bxCurrentList.length > 0 ? (
                                  <>
                                      ...
                                      <div className={`w-full bg-slate-900/90 p-4 md:p-10 rounded-2xl md:rounded-3xl border-2 shadow-2xl mt-6 md:mt-8 ${bxBankLevel === 'b1' ? 'border-emerald-500/50 shadow-[0_0_40px_rgba(16,185,129,0.2)]' : 'border-sky-500/50 shadow-[0_0_40px_rgba(14,165,233,0.3)]'}`}>
                                          <p className="text-gray-400 font-bold mb-2 uppercase tracking-widest text-[10px] md:text-sm"><Icon name="languages" className="w-3 h-3 md:w-4 md:h-4 inline mr-2" />{bxBankLevel === 'b1' ? 'BÃ¡sico / ComÃºn:' : 'Nivel B1:'}</p>
                                          ...
                                          <div className="bg-black/40 p-3 md:p-4 rounded-xl border border-white/10 flex gap-2 md:gap-3">
                                              ...
                                              <p className="text-xs md:text-sm text-gray-200 leading-relaxed"><strong className="text-amber-400 uppercase tracking-wide">La LÃ³gica del Profesor:</strong> <br/> {bxCurrentList[bxIndex]?.trick}</p>
                                          </div>
                                          {bxCurrentList[bxIndex]?._mullerUser ? (
                                              <div className="mt-4 md:mt-5 flex flex-col sm:flex-row flex-wrap gap-2 md:gap-3 items-stretch sm:items-center border-t border-white/10 pt-4">
                                                  ...
                                                  <div className="flex flex-wrap items-center gap-2">
                                                      ...
                                                      <button type="button" onClick={handleBxUserCardMove} className="text-xs md:text-sm bg-emerald-900/70 hover:bg-emerald-800 text-emerald-100 px-3 py-2 rounded-xl font-bold border border-emerald-600/50">Mover aquÃ­</button>
                                                  </div>
                                              </div>
                                          ) : null}
                                      </div>
                                  ...
                                  <div className="text-gray-500 font-bold text-base md:text-xl">CategorÃ­a sin datos (Selecciona otra).</div>
                              )}
                          </div>
                      </div>
                  )}
                  ...
                  {activeTab === 'lexikon' && !practiceActive && (
                      <div className="flex-1 flex flex-col p-4 md:p-8 max-w-4xl mx-auto w-full animate-in fade-in duration-500 overflow-y-auto pb-24">
                          ...
                          <div className="rounded-2xl border border-cyan-700/40 bg-slate-900/80 p-4 md:p-6 mb-6 shadow-xl">
                              ...
                              <div className="flex flex-wrap gap-2 mb-3">
                                  ...
                                  <select value={lexikonDictKind} onChange={(e) => setLexikonDictKind(e.target.value)} className="bg-black/50 border border-cyan-800/60 rounded-xl px-3 py-2 text-white text-sm outline-none max-w-[min(100%,220px)]">
                                      ...
                                      <option value="wiki-de">Solo Wiktionary (alemÃ¡n)</option>
                                      <option value="wiki-es">Solo Wiktionary (espaÃ±ol)</option>
                                  </select>
                                  <button type="button" disabled={lexikonDictLoading} onClick={runLexikonDictionarySearch} className="px-4 py-2 rounded-xl bg-cyan-700 hover:bg-cyan-600 font-bold text-sm disabled:opacity-50">{lexikonDictLoading ? 'â€¦' : 'Buscar'}</button>
                              </div>
                              {lexikonResults && lexikonResults.dictTranslate && !lexikonResults.error ? (
                                  <div className="rounded-xl bg-cyan-950/40 border border-cyan-600/25 p-4 space-y-2">
                                      ...
                                      <div className="flex flex-wrap gap-2 pt-1">
                                          {lexikonResults.tl === 'de' && lexikonResults.out ? (
                                              <a href={`https://de.wiktionary.org/wiki/${encodeURIComponent((lexikonResults.out.split(/[\s,.;]+/)[0] || '').replace(/^[\s"'Â«Â»]+|[\s"'Â»]+$/g, ''))}`} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300 text-xs font-bold underline">Wiktionary DE (1.Âª palabra del resultado)</a>
                                          ) : null}
                                  ...
                                  <p className="text-gray-500 text-sm">Sin resultados. Prueba otra grafÃ­a.</p>
                              ) : null}
                          </div>
                          ...
                          <div className="rounded-2xl border border-indigo-700/40 bg-slate-900/80 p-4 md:p-6 mb-6 shadow-xl">
                              ...
                              <p className="text-[11px] text-gray-500 mb-2">Elige el idioma de <strong className="text-gray-300">salida</strong>; el origen se detecta solo (evita que una palabra espaÃ±ola se traduzca mal por empate DE/ES).</p>
                              <div className="flex flex-wrap gap-2 mb-3">
                              ...
                              <textarea value={lexikonTransText} onChange={(e) => setLexikonTransText(e.target.value)} placeholder="Frase o palabra (cualquier registro)â€¦" className="w-full min-h-[100px] bg-black/50 border border-indigo-800/60 rounded-xl p-3 text-white text-sm outline-none focus:border-indigo-500 mb-3" />
                              <button type="button" disabled={lexikonTransLoading} onClick={runLexikonTranslate} className="mb-4 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 font-bold text-sm disabled:opacity-50">{lexikonTransLoading ? 'Traduciendoâ€¦' : 'Traducir'}</button>
                              {lexikonTransOut ? (
                              ...
                              <div className="border-t border-white/10 pt-4 space-y-3">
                                  <p className="text-[10px] font-bold text-amber-300 uppercase">Guardar par en lecciÃ³n (Vocab)</p>
                                  <div className="flex flex-wrap gap-2 items-center">
                                      <select value={lexikonSaveLessonId} onChange={(e) => setLexikonSaveLessonId(e.target.value)} className="bg-black/60 border border-amber-800/60 text-white text-sm rounded-lg px-3 py-2 outline-none flex-1 min-w-[180px]">
                                          ...
                                          <option value="__new__">+ Nueva lecciÃ³nâ€¦</option>
                                          {customVocabLessons.map((l) => (
                                              <option key={l.id} value={l.id}>{l.title} ({l.words?.length || 0} pal.)</option>
                                          ))}
                                      </select>
                                      {lexikonSaveLessonId === '__new__' ? (
                                          <input type="text" value={lexikonNewLessonTitle} onChange={(e) => setLexikonNewLessonTitle(e.target.value)} placeholder="TÃ­tulo nueva lecciÃ³n" className="bg-black/50 border border-amber-800/60 rounded-lg px-3 py-2 text-white text-sm flex-1 min-w-[140px]" />
                                      ) : null}
                                  </div>
                                  <div className="flex flex-wrap gap-2">
                                      <label className="flex-1 min-w-[120px] text-[10px] text-gray-500 block">AlemÃ¡n<input type="text" value={lexikonPairDe} onChange={(e) => setLexikonPairDe(e.target.value)} className="mt-0.5 w-full bg-black/50 border border-emerald-800/50 rounded-lg px-2 py-1.5 text-sm text-emerald-100" placeholder="der Bahnhofâ€¦" /></label>
                                      <label className="flex-1 min-w-[120px] text-[10px] text-gray-500 block">EspaÃ±ol<input type="text" value={lexikonPairEs} onChange={(e) => setLexikonPairEs(e.target.value)} className="mt-0.5 w-full bg-black/50 border border-sky-800/50 rounded-lg px-2 py-1.5 text-sm text-sky-100" placeholder="la estaciÃ³nâ€¦" /></label>
                                  </div>
                                  <button type="button" onClick={() => appendPairToCustomLesson(lexikonPairDe, lexikonPairEs)} className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-amber-700 hover:bg-amber-600 font-bold text-sm border border-amber-500/40">Guardar par DE / ES en la lecciÃ³n</button>
                  ...
                  {/* PROGRESO (con botones de sincronizaciÃ³n aÃ±adidos en barra) */}
                      ...
                      <div className="p-4 md:p-8 max-w-5xl mx-auto w-full animate-in fade-in duration-500">
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
                              <div className="rounded-2xl border border-sky-500/35 bg-gradient-to-br from-sky-950/70 to-slate-900/80 p-4 md:p-5 shadow-xl">
                                  ...
                                  <p className="text-[10px] text-gray-500 uppercase tracking-wider pt-3 mb-2">Retos del dÃ­a (+5 monedas, una vez al dÃ­a)</p>
                                  <div className="flex flex-wrap gap-2">
                              ...
                              <div className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-950/40 to-yellow-950/20 p-4 md:p-5 shadow-xl">
                                  <h2 className="text-xs font-black text-amber-200 uppercase tracking-wider mb-3">Resumen rÃ¡pido</h2>
                                  <div className="space-y-2 text-xs text-gray-300">
                                      <p><span className="text-white font-bold">Racha (honesta):</span> {userStats.streakDays} dÃ­as</p>
                                      ...
                                      <p><span className="text-white font-bold">PronunciaciÃ³n:</span> {userStats.pronunciationAttempts || 0} intentos</p>
                                      <p><span className="text-white font-bold">Diktat:</span> {userStats.diktatCorrect || 0} / {userStats.diktatAttempts || 0}</p>
                                      <p className="text-[10px] text-gray-500 pt-1">Exporta PDF desde el botÃ³n de abajo.</p>
                                  </div>
                              </div>
                          </div>
                          ...
                          <div className="bg-gray-800/80 p-4 md:p-6 rounded-2xl border border-gray-700 shadow-xl mb-6 md:mb-8">
                              ...
                              <p className="text-[10px] md:text-xs text-gray-500 mb-2">Basada en tus sesiones reales (vocab, diktat, prÃ¡ctica). Si un dÃ­a estÃ¡ vacÃ­o, la barra es baja.</p>
                          ...
                          {mullerProgresoSnapshot && (
                              <div className="bg-gradient-to-r from-purple-950/80 to-indigo-950/80 p-4 md:p-6 rounded-2xl border border-purple-700/50 shadow-xl mb-6 md:mb-8">
                                  <h2 className="text-base md:text-lg font-bold text-purple-200 mb-3 flex items-center gap-2"><Icon name="graduation-cap" className="w-5 h-5 md:w-6 md:h-6" /> Entrenamiento MÃ¼ller (avanzado)</h2>
                                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:gap-3 text-xs md:text-sm">
                                      ...
                                      <div className="bg-black/30 rounded-lg p-2 text-center border border-white/10"><span className="text-gray-400 block">PrecisiÃ³n</span><span className="font-black text-emerald-300 text-lg">{mullerProgresoSnapshot.accuracy}%</span></div>
                                  ...
                                  <p className="text-[10px] text-gray-500 mt-3">Abre la pestaÃ±a Entrenamiento para practicar artÃ­culos, verbos y preposiciones.</p>
                              </div>
                          )}
                          <p className="text-xs text-emerald-200/90 mb-4 flex flex-wrap items-center gap-2"><Icon name="timer" className="w-4 h-4" /> Repaso espaciado (SRS): <strong>{Object.keys(vocabSrsMap).length}</strong> palabras con fecha de repaso Â· se ordenan solas al abrir una lecciÃ³n. Solo en este navegador.</p>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
                              <div className="bg-gray-900 p-4 md:p-6 rounded-2xl border border-red-900/50 flex flex-col relative group">
                                  <h2 className="text-base md:text-lg font-bold mb-3 md:mb-4 flex items-center gap-2 text-red-500"><Icon name="target" className="w-5 h-5 md:w-6 md:h-6" /> Vocab. DifÃ­cil</h2>
                                  <ul className="space-y-1 md:space-y-2 flex-1 overflow-y-auto pr-2 mb-3 md:mb-4 max-h-32 md:max-h-40">
                                      {!userStats.difficultVocab || userStats.difficultVocab.length === 0 ? <p className="text-gray-500 text-xs md:text-sm">VacÃ­o.</p> : 
                              ...
                              <div className="bg-gray-900 p-4 md:p-6 rounded-2xl border border-blue-900/50 flex flex-col">
                                  ...
                                  <ul className="space-y-1 md:space-y-2 flex-1 overflow-y-auto pr-2 mb-3 md:mb-4 max-h-32 md:max-h-40">
                                      {!userStats.normalVocab || userStats.normalVocab.length === 0 ? <p className="text-gray-500 text-xs md:text-sm">VacÃ­o.</p> : 
                              ...
                              <div className="bg-gray-900 p-4 md:p-6 rounded-2xl border border-cyan-900/50 flex flex-col">
                                  <h2 className="text-base md:text-lg font-bold mb-3 md:mb-4 flex items-center gap-2 text-cyan-400"><Icon name="brain" className="w-5 h-5 md:w-6 md:h-6" /> GramÃ¡tica</h2>
                                  <ul className="space-y-1 md:space-y-2 flex-1 overflow-y-auto pr-2 mb-3 md:mb-4 max-h-32 md:max-h-40">
                                      {!userStats.difficultGrammar || userStats.difficultGrammar.length === 0 ? <p className="text-gray-500 text-xs md:text-sm">VacÃ­o.</p> : 
                      ...
                      <div className="p-4 md:p-8 max-w-7xl mx-auto w-full flex flex-col md:flex-row gap-6 md:gap-8 min-h-full animate-in fade-in">
                          <div className="flex-[2] bg-gray-800/50 p-4 md:p-6 rounded-2xl border border-gray-700 flex flex-col max-h-full overflow-y-auto hide-scrollbar">
                              <div className="bg-purple-900/30 border border-purple-500/50 p-4 md:p-5 rounded-xl mb-4 md:mb-6">
                                  ...
                                  <p className="text-xs md:text-sm text-gray-300 mb-3">Copia este prompt en tu IA favorita para que te genere diÃ¡logos perfectos y compatibles con el Entrenador MÃ¼ller.</p>
    ...
    "Eres un profesor de alemÃ¡n experto en TELC B1. Genera un diÃ¡logo EXTENSO (sin lÃ­mite de lÃ­neas) sobre [TEMA]. IMPORTANTE: marca los Redemittel clave solo con [R] al final de la lÃ­nea alemana. No uses la palabra NÃ¼tzlich ni sÃ­mbolos raros.<br/><br/>Formato: Nombre: Frase en alemÃ¡n. (TraducciÃ³n) [palabra - traducciÃ³n] [R]"
</p>
<button 
    ...
    onClick={() => navigator.clipboard.writeText(`Eres un profesor de alemÃ¡n experto en TELC B1. Genera un diÃ¡logo EXTENSO (sin lÃ­mite de lÃ­neas) sobre [TEMA]. IMPORTANTE: marca los Redemittel clave solo con [R] al final de la lÃ­nea alemana. No uses la palabra NÃ¼tzlich ni sÃ­mbolos raros.\n\nFormato: Nombre: Frase en alemÃ¡n. (TraducciÃ³n) [palabra - traducciÃ³n] [R]`)}
>
    <Icon name="copy" className="w-3 h-3 md:w-4 md:h-4" />
                              ...
                              <h2 className="text-xl md:text-2xl font-bold flex items-center gap-2 text-purple-400"><Icon name="edit-3" className="w-5 h-5 md:w-6 md:h-6" /> AÃ±adir Guion Copiado</h2>
                              ...
                              <div className="mt-6 md:mt-8 border-t border-gray-600/80 pt-5 md:pt-6">
                                  ...
                                  <p className="text-[11px] md:text-xs text-gray-400 mb-3 leading-relaxed">Pega un guion (<code className="text-gray-300">Nombre:</code> â€¦), listas <code className="text-gray-300">alemÃ¡n - espaÃ±ol</code> o lÃ­neas sueltas. La app clasifica cada trozo en <strong className="text-gray-200">vocabulario, verbos, preposiciones, conectores o Redemittel</strong> y, con <strong className="text-gray-200">nivel automÃ¡tico</strong>, estima si va a B1 o B2 (heurÃ­stica local, no IA). Si el cuadro estÃ¡ vacÃ­o, se usa el texto del guion de arriba. Tus aportaciones se guardan en el navegador y se <strong className="text-gray-200">mezclan</strong> con las tarjetas del archivo <code className="text-gray-300">b1-b2-database.json</code> (esas no se borran con el botÃ³n rojo).</p>
                                  <p className="text-[10px] md:text-xs text-indigo-300/90 mb-3 leading-relaxed bg-indigo-950/25 border border-indigo-500/20 rounded-lg px-3 py-2">Si en <strong className="text-white">Historia</strong> tenÃ­as cargado un guion <strong className="text-white">guardado en esta lista</strong>, al pulsar Distribuir las frases quedan <strong className="text-white">vinculadas a ese guion</strong>. Al borrar el guion con la papelera, te preguntamos si quieres quitar tambiÃ©n esas tarjetas en B1/B2. Lo que enviaste con el ejemplo por defecto o sin guion cargado no se vincula (Ãºsalo Â«Borrar mis aportacionesÂ» para vaciar todo lo tuyo).</p>
                                  <textarea className="w-full min-h-[100px] md:min-h-[120px] bg-black/50 border border-emerald-800/60 p-3 rounded-lg text-white font-mono text-xs md:text-sm mb-3 outline-none focus:border-emerald-500 resize-none" placeholder="Pega aquÃ­ o dÃ©jalo vacÃ­o para usar el cuadro del guion de arribaâ€¦" value={bxImportText} onChange={(e) => setBxImportText(e.target.value)} />
                                  <div className="flex flex-wrap gap-2 mb-3">
                          ...
                          <div className="w-full md:w-1/3 flex flex-col gap-4 max-h-[600px] md:max-h-full">
                              ...
                              <div className="bg-amber-900/20 p-4 md:p-5 rounded-2xl border border-amber-800/50 flex flex-col flex-1 min-h-0">
                                  <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                                      <h2 className="text-lg md:text-xl font-bold text-amber-400 flex items-center gap-2"><Icon name="library" className="w-5 h-5 md:w-6 md:h-6" /> AÃ±adir Vocabulario</h2>
                                  ...
                                  <div className="bg-black/40 p-2 md:p-3 rounded-xl border border-amber-900/50 mb-3 md:mb-4 flex flex-col gap-2 shrink-0">
                                      <div className="flex gap-2 items-center">
                                          <input type="text" placeholder="TÃ­tulo (Ej: Lektion 12)" className="w-2/3 bg-black/50 border border-amber-800 p-1.5 md:p-2 rounded-lg text-white text-xs md:text-sm outline-none focus:border-amber-500" value={vocabTitleInput} onChange={(e) => setVocabTitleInput(e.target.value)} />
                                  ...
                                  <textarea className="w-full h-12 md:h-14 bg-black/50 border border-amber-800 p-2 rounded-lg text-white font-mono text-[10px] md:text-xs outline-none focus:border-amber-500 resize-none" placeholder="Pega aquÃ­ tu lista...&#10;der Apfel - la manzana" value={vocabTextInput} onChange={(e) => setVocabTextInput(e.target.value)} />
                                  </div>
                                  <div className="flex-1 overflow-y-auto space-y-2 pr-2 hide-scrollbar">
                                      {customVocabLessons.length === 0 ? <p className="text-gray-500 text-xs italic">Tus lecciones guardadas aparecerÃ¡n aquÃ­.</p> : null}
                                      {customVocabLessons.map(lesson => (
                  ...
                  {/* VOCABULARIO (con botÃ³n de escritura a mano) */}
                  {activeTab === 'vocabulario' && !practiceActive && (
                      <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 animate-in fade-in relative">
                          ...
                          <div className="w-full max-w-md mb-4 mt-10 md:mt-6 px-3">
                              ...
                              <p className="text-[9px] text-amber-900/50 mt-1">Hoy: {mullerGetStreakTodayStats().vocabRated || 0}/{mainDailyGoal} Â· al completar el objetivo +10 monedas (una vez al dÃ­a)</p>
                          </div>
                                  ...
                                  {!showVocabTranslation ? (
                                      ...
                                      <div className="flex flex-col items-center gap-6 md:gap-8 w-full animate-in slide-in-from-bottom-8">
                                          ...
                                          <div className="flex flex-col md:flex-row gap-3 md:gap-4 w-full">
                                              <button onClick={() => handleVocabDifficulty('easy')} className="flex-1 bg-green-600 hover:bg-green-500 text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg shadow-lg transition">FÃ¡cil (Descartar)</button>
                                              ...
                                              <button onClick={() => handleVocabDifficulty('hard')} className="flex-1 bg-red-600 hover:bg-red-500 text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg border-2 border-red-400 transition">DifÃ­cil</button>
                                          </div>
                                          <p className="text-[10px] md:text-xs text-slate-800/80 text-center max-w-md leading-snug">Cada opciÃ³n actualiza el calendario SRS (intervalos tipo Anki/SM-2): mÃ¡s espacio si fue fÃ¡cil, repaso antes si fue difÃ­cil.</p>
                                      </div>
                                  )}
                                  ...
                                  {(() => {
                                      ...
                                      return (
                                          <div className="text-[10px] text-amber-900/70 -mt-2 text-center max-w-lg space-y-0.5">
                                              ...
                                              <p>Vistas: <strong>{vc}</strong> Â· Calificaciones: <strong>{rc}</strong></p>
                                          </div>
                                      );
                                  })()}
                              </div>
                  ...
                  {activeTab === 'shadowing' && !practiceActive && (
                      <div className="flex-1 flex flex-col p-4 md:p-8 max-w-3xl mx-auto w-full animate-in fade-in duration-500 overflow-y-auto">
                          ...
                          <p className="text-teal-50/90 text-sm md:text-base mb-6 leading-relaxed">
                              Escucha el modelo en alemÃ¡n (voz del sistema), luego repite en voz alta siguiendo el ritmo y la entonaciÃ³n. Es la tÃ©cnica de <strong className="text-white">shadowing</strong> para ganar fluidez y pronunciaciÃ³n tipo TELC.
                          </p>
                          {guionData.length === 0 ? (
                                  ...
                                  {(() => {
                                      ...
                                      return (
                                          <>
                                              ...
                                              <p className="text-center text-gray-500 text-xs mb-4">DespuÃ©s de escuchar: habla al mismo tiempo o justo despuÃ©s; prioriza fluidez, no perfecciÃ³n.</p>
                                              <div className="rounded-2xl border border-teal-500/30 bg-black/35 p-4 md:p-5 mb-5">
                                                  <p className="text-xs font-bold text-teal-300 mb-1 flex items-center gap-2"><Icon name="mic" className="w-4 h-4" /> Comprobar pronunciaciÃ³n</p>
                                                  <p className="text-[10px] text-gray-500 mb-3">El navegador transcribe tu alemÃ¡n y lo compara con el texto del guion (sin juzgar acento puro). MantÃ©n pulsado el micrÃ³fono mientras lees la frase; suelta para ver el resultado. <span className="text-teal-400/90">En Shadowing no pierdes vidas.</span></p>
                                                  <div className="flex flex-col items-center gap-3">
                                                      <button type="button" aria-label="MantÃ©n pulsado y lee en voz alta para comprobar pronunciaciÃ³n" onMouseDown={micMouseDownGuard(() => scene?.text && handleVoiceStart(scene.text, { mode: 'shadow' }))} onMouseUp={handleVoiceStop} onMouseLeave={handleVoiceStop} onTouchStart={micTouchStartGuard(() => scene?.text && handleVoiceStart(scene.text, { mode: 'shadow' }))} onTouchEnd={handleVoiceStop} className={`rounded-full p-5 md:p-6 text-white transition shadow-xl select-none touch-manipulation ${isListening ? 'bg-red-500 animate-pulse ring-4 ring-red-400/35 shadow-[0_0_32px_rgba(239,68,68,0.55)] border-2 border-white/35' : 'muller-mic-hold-btn ring-4 ring-teal-300/35 shadow-[0_0_28px_rgba(20,184,166,0.45)]'}`} title="MantÃ©n pulsado y lee en voz alta">
                                                          <Icon name="mic" className="w-10 h-10 md:w-12 md:h-12 text-white relative z-[1]" />
                                                      </button>
                                                      <span className="text-[10px] text-gray-500">MantÃ©n pulsado Â· suelta para evaluar</span>
                                                      {grammarPolizeiMsg && (
                  ...
                  {activeTab === 'lectura' && !practiceActive && (
                      <div className="flex-1 flex flex-col p-4 md:p-8 max-w-4xl mx-auto w-full animate-in fade-in duration-500 overflow-y-auto">
                          ...
                          <p className="text-sky-50/90 text-sm md:text-base mb-4 leading-relaxed">
                              Lee un texto completo y compara tu producciÃ³n con el original. Puedes usar la historia actual, un guion guardado o pegar un texto manualmente.
                              Toca cualquier palabra para ver traducciÃ³n al espaÃ±ol y, si es verbo, pasado y Perfekt.
                          </p>
                              ...
                              {readingSource === 'one_saved' && (
                                  <label className="text-xs font-bold text-sky-200/90 uppercase tracking-wider md:col-span-2">
                                      ...
                                      <select value={readingScriptId} onChange={(e) => setReadingScriptId(e.target.value)} className="mt-1 w-full bg-black/40 border border-white/15 rounded-lg px-3 py-2 text-sm text-white normal-case">
                                          <option value="__current__" disabled>Selecciona un guionâ€¦</option>
                          ...
                          {readingSource === 'paste' && (
                              <textarea value={readingTextInput} onChange={(e) => setReadingTextInput(e.target.value)} placeholder="Pega aquÃ­ tu texto en alemÃ¡nâ€¦" className="w-full h-40 bg-black/35 border border-sky-500/30 rounded-xl p-3 text-sm md:text-base text-white mb-3" />
                          )}
                          ...
                          <div className="rounded-xl border border-cyan-500/30 bg-cyan-950/25 p-3 mb-3 space-y-2">
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                  <p className="text-[11px] font-black text-cyan-200 uppercase tracking-wider">PDF estudio (premium Â· MVP)</p>
                                  <span className="text-[10px] text-cyan-100/75">Texto por pÃ¡gina + OCR fallback</span>
                              </div>
                              <div className="flex flex-wrap gap-2 items-center">
                                  ...
                                  {pdfStudySavedDocs.length > 0 && (
                                      <>
                                          ...
                                          >
                                              <option value="__none__">Biblioteca PDFâ€¦</option>
                                              {pdfStudySavedDocs.map((d) => (
                                                  <option key={d.id} value={d.id}>
                                                      {d.name} Â· {d.totalPages || 0}p
                                                  </option>
                                              ))}
                                          </select>
                                          <button
                                  ...
                                  {pdfStudyDoc && (
                                      <>
                                          ...
                                          >
                                              {(pdfStudyDoc.pages || []).map((p) => (
                                                  <option key={p.page} value={p.page}>
                                                      PÃ¡gina {p.page}{p.unit ? ` Â· U:${p.unit}` : ''}{p.lesson ? ` Â· L:${p.lesson}` : ''}{p.ocrPending ? ' Â· OCR pendiente' : ''}
                                                  </option>
                                              ))}
                                          </select>
                                          <button
                                          ...
                                          >
                                              {pdfStudyOcrBusy ? 'OCRâ€¦' : 'OCR pÃ¡gina'}
                                          </button>
                                          <button
                                              type="button"
                              ...
                              {pdfStudyDoc && (
                                  <div className="text-[10px] text-cyan-100/80 space-y-1">
                                      <p><strong className="text-cyan-200">Libro:</strong> {pdfStudyDoc.name} Â· {pdfStudyDoc.totalPages} pÃ¡ginas</p>
                              ...
                              {pdfStudyDoc && (
                                  <textarea
                                      ...
                                      placeholder="Sin texto extraÃ­do en esta pÃ¡gina todavÃ­a."
                                  />
                              )}
                          </div>
                          ...
                          {readingFocusMode && (
                                  <p className="text-[11px] text-emerald-200/90 mb-2">
                                      Modo lectura activo: se ocultan resultados de evaluaciÃ³n para concentrarte en comprensiÃ³n + diccionario.
                                  </p>
                              )}
                              {readingTargetText ? (
                                  <div
                                      ...
                                      {readingWordTokens.map((token, idx) => {
                                          ...
                                          return (
                                              <span
                                                  ...
                                                  title="Tocar para ver traducciÃ³n y tiempos"
                                              >
                                                  {token.text}
                                              </span>
                                          );
                              ...
                              {readingSelectedSnippet && (
                                  <div className="mt-2 rounded-lg border border-teal-500/30 bg-teal-950/25 px-2.5 py-2">
                                      <p className="text-[11px] text-teal-100">
                                          Frase seleccionada: <span className="text-white">{readingSelectedSnippet.length > 160 ? `${readingSelectedSnippet.slice(0, 160)}â€¦` : readingSelectedSnippet}</span>
                                      </p>
                                  </div>
                              )}
                          </div>
                          ...
                          {readingWordInfo && (
                              <div className="rounded-xl bg-cyan-950/35 border border-cyan-500/30 p-3 mb-4 space-y-2">
                                  <div className="flex items-center justify-between gap-2">
                                      ...
                                      {readingWordInfo.loading && <span className="text-[10px] font-bold text-cyan-300 animate-pulse">Buscandoâ€¦</span>}
                                  </div>
                                  {readingWordInfo.translation && (
                                      <p className="text-sm text-cyan-100">
                                          <span className="font-bold text-cyan-300">TraducciÃ³n:</span> {readingWordInfo.translation}
                                      </p>
                                  )}
                                  {!readingWordInfo.loading && !readingWordInfo.translation && (
                                      <p className="text-xs text-cyan-200/80">No se obtuvo traducciÃ³n automÃ¡tica para esta palabra.</p>
                                  )}
                                  ...
                                  {readingVerbInfo && (
                                      <div className="rounded-lg bg-black/25 border border-white/10 p-2.5">
                                          <p className="text-xs text-emerald-300 font-black mb-1">
                                              Verbo detectado{readingVerbInfo.level ? ` Â· ${readingVerbInfo.level}` : ''}
                                          </p>
                                          ...
                                          {readingVerbInfo.praeteritum && <p className="text-xs text-emerald-100">Pasado (PrÃ¤teritum): {readingVerbInfo.praeteritum}</p>}
                          ...
                          <div className="flex flex-wrap gap-3 items-center mb-3">
                              ...
                              <button type="button" onClick={stopReadingListen} disabled={!readingListening} className="px-4 py-2 rounded-xl bg-red-700 hover:bg-red-600 disabled:opacity-40 font-black text-sm">â¹ Parar y evaluar</button>
                              <span className={`text-xs font-bold ${readingListening ? 'text-emerald-300 animate-pulse' : 'text-gray-500'}`}>{readingListening ? 'Escuchandoâ€¦' : 'MicrÃ³fono en espera'}</span>
                          </div>
                      ...
                      <div className="flex-1 flex flex-col p-3 md:p-6 max-w-4xl mx-auto w-full animate-in fade-in duration-500 overflow-y-auto pb-24">
                          ...
                          {ocrHistoryList.length > 0 && (
                              <div className="mb-4 rounded-xl border border-indigo-500/35 bg-indigo-950/20 p-3">
                                  <p className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider mb-2">Historial OCR (Ãºltimas {ocrHistoryList.length})</p>
                                  <ul className="space-y-1.5 text-[10px] text-gray-400 max-h-36 overflow-y-auto pr-1">
                                      {ocrHistoryList.map((h) => (
                                          <li key={h.id || h.at} className="flex flex-wrap items-baseline justify-between gap-2 border-b border-white/5 pb-1.5">
                                              ...
                                              <span className="w-full text-gray-500 truncate italic">{(h.textSnippet || '').slice(0, 70)}{(h.textSnippet || '').length > 70 ? 'â€¦' : ''}</span>
                                          </li>
                                      ))}
                                  </ul>
                              </div>
                          ...
                          {writingMode === 'free' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4">
                                  <p className="text-rose-200/90 text-sm font-bold mb-1">PÃ¡gina en blanco</p>
                                  <p className="text-[11px] text-gray-500">Escribe lo que quieras: resÃºmenes, listas, conjugacionesâ€¦ Usa <strong className="text-gray-300">Borrar</strong> o <strong className="text-gray-300">Guardar PNG</strong> debajo del lienzo.</p>
                              </div>
                          )}

                          {writingMode === 'copy' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4 space-y-2">
                                  <p className="text-rose-200/90 text-sm font-bold">Copia la frase (caligrafÃ­a alemana)</p>
                          ...
                          {writingMode === 'dictation' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4 space-y-3">
                                  <p className="text-rose-200/90 text-sm font-bold">Dictado alemÃ¡n</p>
                                  ...
                                  <p className="text-[10px] text-rose-200/75">
                                      Ãtem {Math.min(writingDictIdx + 1, Math.max(1, writingDictationPool.length))} de {Math.max(1, writingDictationPool.length)}
                                      {writingDictationPool[writingDictIdx % Math.max(1, writingDictationPool.length)]?.origin ? ` Â· ${writingDictationPool[writingDictIdx % Math.max(1, writingDictationPool.length)].origin}` : ''}
                                  </p>
                                  <div className="flex flex-wrap gap-2 items-center">
                                      <button
                                          type="button"
                                      ...
                                      >
                                          {writingDictReveal ? 'Ocultar soluciÃ³n' : 'Mostrar soluciÃ³n'}
                                      </button>
                                  </div>
                                  {writingDictReveal && writingDictationPool.length > 0 && (
                                      <div className="border border-emerald-700/40 rounded-lg p-4 md:p-5 bg-emerald-950/40">
                                          ...
                                          <p className="text-emerald-200/90 text-sm md:text-base mt-2 italic leading-relaxed">
                                              {writingDictationPool[writingDictIdx % writingDictationPool.length].es || 'Sin traducciÃ³n disponible en esta lÃ­nea.'}
                                          </p>
                                      </div>
                                  )}
                              </div>
                          ...
                          {writingMode === 'prompt' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4 space-y-2">
                                  <p className="text-rose-200/90 text-sm font-bold">Tema para redacciÃ³n corta</p>
                          ...
                          {writingMode === 'telc' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-orange-500/30 p-3 md:p-4 space-y-3">
                                  ...
                                  <p className="text-[11px] text-gray-500">
                                      Redacta a mano como en examen real con bolÃ­grafo Ã³ptico. Cubre todos los puntos del enunciado.
                                  </p>
                                  <div className="rounded-lg border border-orange-500/35 bg-orange-950/25 p-2.5 space-y-2">
                                      <p className="text-[10px] font-bold uppercase tracking-wider text-orange-200">MÃ©todo de escritura TELC</p>
                                      <div className="flex flex-wrap gap-2">
                                          <button
                                              type="button"
                                      ...
                                      {writingTelcInputMode === 'keyboard' && (
                                          <textarea
                                              ...
                                              placeholder="Escribe aquÃ­ tu carta/email TELC con tecladoâ€¦"
                                      ...
                                      {writingTelcInputMode === 'pen' && (
                                          <p className="text-[11px] text-orange-100/90">
                                              Usa el lienzo de abajo y luego pulsa <strong>Evaluar texto TELC</strong> (leerÃ¡ el OCR).
                                          </p>
                                      )}
                                  </div>
                                  <p className="text-[10px] text-rose-200/75">
                                      Tarea {Math.min(writingTelcIdx + 1, Math.max(1, WRITING_TELC_TASKS.length))} de {Math.max(1, WRITING_TELC_TASKS.length)} Â· {WRITING_TELC_TASKS[writingTelcIdx % Math.max(1, WRITING_TELC_TASKS.length)]?.level || 'TELC'}
                                  </p>
                                  ...
                                  {writingTelcCoach && (
                                      <div className="rounded-xl border border-emerald-500/35 bg-emerald-950/25 p-3 space-y-2">
                                          <div className="flex flex-wrap items-center justify-between gap-2">
                                              ...
                                              <span className="text-xs font-black text-white bg-black/35 border border-white/10 rounded-full px-2 py-0.5">{writingTelcCoach.total}/{writingTelcCoach.max} Â· {writingTelcCoach.pct}%</span>
                                          </div>
                                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px]">
                                              ...
                                              <p className="rounded-lg bg-black/30 border border-white/10 px-2 py-1"><span className="text-gray-400">KohÃ¤renz</span><br /><strong className="text-emerald-200">{writingTelcCoach.scoreCohesion}/5</strong></p>
                          ...
                          {writingMode === 'letters' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4 space-y-2">
                                  ...
                                  <p className="text-lg md:text-xl text-white tracking-wide font-medium">PrÃ¡ctica: {LETTER_DRILLS[writingLetterIdx % LETTER_DRILLS.length].practice}</p>
                          ...
                          {writingMode === 'guion' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4 space-y-2">
                                  {guionData.length === 0 ? (
                                      ...
                                      <>
                                          <p className="text-rose-200/90 text-sm font-bold">Copia una lÃ­nea del guion activo</p>
                          ...
                          {writingMode === 'vocab' && (
                              <div className="mb-4 rounded-xl bg-black/35 border border-rose-500/25 p-3 md:p-4 space-y-2">
                                  ...
                                  {currentVocabList.length === 0 ? (
                                      <p className="text-gray-500 text-sm">Sin lista de vocabulario aÃºn. Abre <strong className="text-white">Vocab</strong> o carga un guion con vocabulario en <strong className="text-white">Historia</strong>.</p>
                                  ) : (
                                      <>
                          ...
                          <div className="flex flex-wrap gap-2 md:gap-3 items-center mb-3 text-[11px] md:text-xs">
                              ...
                              <span className="text-gray-500">Grosor lÃ¡piz:</span>
                              {[2, 4, 7].map((w) => (
                          ...
                          <div className="relative max-w-md w-full rounded-[2rem] border border-white/20 bg-gradient-to-br from-fuchsia-900/95 via-violet-950/95 to-indigo-950/95 p-8 shadow-[0_0_80px_rgba(236,72,153,0.35)] text-center overflow-hidden animate-in zoom-in-95 duration-300">
                              ...
                              <div className="relative z-10">
                                  ...
                                  <div className="flex flex-wrap justify-center gap-3 text-sm font-bold">
                                      ...
                                      {celebrationModal.milestone ? <span className="rounded-full bg-emerald-500/30 px-4 py-2 text-emerald-100">Bonus Â· 3 lecciones</span> : null}
                                  ...
                                  <button type="button" className="mt-8 w-full rounded-xl bg-white text-violet-950 font-black py-3 hover:bg-fuchsia-100 transition" onClick={() => setCelebrationModal(null)}>Â¡Genial!</button>
                              </div>
                          </div>
                      </div>
                  )}
                  ...
                  {activeTab === 'ruta' && !practiceActive && (
                      <div className="flex-1 flex flex-col overflow-y-auto hide-scrollbar p-4 md:p-8 max-w-4xl mx-auto w-full animate-in fade-in duration-500">
                          ...
                          <div className="flex flex-wrap items-center gap-3 mb-4 rounded-2xl border border-white/10 bg-black/35 p-4">
                              ...
                              <button type="button" onClick={() => { try { localStorage.setItem('muller_sfx_enabled', (typeof window.__mullerSfxEnabled === 'function' && window.__mullerSfxEnabled()) ? '0' : '1'); } catch (e) {} setSfxEpoch((x) => x + 1); }} className="ml-auto text-xs font-bold rounded-xl border border-white/15 px-3 py-2 text-gray-300 hover:bg-white/10" title="Acierto, fallo y fanfarria cada 5 aciertos seguidos (5, 10, 15â€¦)">
                                  Sonidos: {sfxEpoch >= 0 && typeof window.__mullerSfxEnabled === 'function' && window.__mullerSfxEnabled() ? 'ON' : 'OFF'}
                              </button>
                          </div>
                          <p className="text-[11px] text-gray-500 mb-4">Tiempo en Ruta (aprox.): {Math.round((rutaProgress.playTimeMs || 0) / 60000)} min Â· Lecciones completadas: {rutaProgress.lessonsCompleted || 0}</p>
                              ...
                              <>
                                  ...
                                  {rutaSubTab === 'camino' && (
                                      <div className="space-y-6">
                                          <div className="rounded-2xl border border-fuchsia-500/25 bg-slate-900/45 p-3 md:p-4">
                                              <p className="text-[11px] font-bold uppercase tracking-wider text-fuchsia-300 mb-2">SecciÃ³n temÃ¡tica</p>
                                              <div className="flex flex-wrap gap-2">
                                                  {[
                                                      { id: 'all', label: 'Todo' },
                                                      { id: 'presentacion', label: 'PresentaciÃ³n' },
                                                      ...
                                                      {lv.lessons
                                                          ...
                                                          return (
                                                              <button key={lesson.id} type="button" disabled={!unlocked} onClick={() => { if (!unlocked) return; setRutaRun({ levelIdx, lessonIdx: origIdx, step: 0 }); setRutaFillInput(''); setRutaTranscript(''); setRutaSpeakErr(''); }} className={`flex flex-wrap items-center justify-between gap-2 rounded-xl border px-4 py-3 text-left transition ${!unlocked ? 'opacity-40 cursor-not-allowed border-white/5' : done ? 'border-emerald-500/40 bg-emerald-950/30 hover:bg-emerald-900/40' : 'border-white/15 bg-black/30 hover:bg-fuchsia-950/40'}`}>
                                                                  ...
                                                                  <span className="text-xs font-bold text-amber-300">+{lesson.rewardCoins} Â· {lesson.rewardXp} XP</span>
                                  ...
                                  {rutaSubTab === 'gramatica' && (
                                      <div className="space-y-4">
                                          ...
                                          <p className="text-xs text-gray-500">Ampliaremos con mÃ¡s temas y ejemplos conforme aÃ±adas contenido al camino.</p>
                                      </div>
                                  )}
  {rutaSubTab === 'test' && (
    ...
    {!placementQuestions.length ? (
      <>
        ...
        <p className="text-gray-300 text-sm">
          Responde unas preguntas para evaluar tu nivel de alemÃ¡n. El test se adapta a tus respuestas.
        </p>
      ...
      <>
          ...
          Responde con sinceridad. El test se adapta para encontrar tu nivel real.
                              ...
                              return (
                                  <div className="rounded-2xl border border-fuchsia-500/30 bg-black/40 p-5 md:p-8">
                                      ...
                                      <p className="text-xs text-fuchsia-400/90 mb-6">{lv.badge} Â· {lv.title}</p>
                                      {st === 0 && (
                                          <>
                                      ...
                                      {st === 2 && lesson.speak && (
                                          <>
                                              <p className="text-gray-300 mb-2">Lee en voz alta en alemÃ¡n:</p>
                                              ...
                                              <div className="flex flex-wrap gap-2 mb-4">
                                                  ...
                                                  <button type="button" disabled={rutaListening} onClick={startRutaListen} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white hover:bg-rose-500 disabled:opacity-50">{rutaListening ? 'Escuchandoâ€¦' : 'Grabar'}</button>
                                              </div>
                                              ...
                                              <p className="text-[11px] text-gray-500 mt-2">Si hoy no puedes usar micrÃ³fono, puedes avanzar igualmente y practicar voz despuÃ©s.</p>
                                          </>
                                      )}
                                  </div>
                              );
                  ...
                  {activeTab === 'inicio' && !practiceActive && (
                      <div className="flex-1 flex flex-col overflow-y-auto hide-scrollbar p-4 md:p-8 max-w-5xl mx-auto w-full animate-in fade-in duration-500">
                          <div className="mb-6 md:mb-8">
                              ...
                              <p className="text-gray-400 text-sm md:text-base max-w-2xl">Elige quÃ© practicar. Tecla <kbd className="px-1.5 py-0.5 rounded bg-white/10 border border-white/15 font-mono text-xs">I</kbd> para volver aquÃ­.</p>
                          </div>
                          <div className="mb-4 rounded-2xl border border-cyan-500/30 bg-cyan-950/35 p-4">
                              <div className="flex items-center justify-between gap-2 mb-2">
                                  <p className="text-cyan-200 font-black text-sm flex items-center gap-2"><Icon name="stethoscope" className="w-4 h-4" /> DiagnÃ³stico rÃ¡pido</p>
                              ...
                              <div className="mb-2 flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] text-cyan-200/80">Comprueba rÃ¡pido que todo lo crÃ­tico estÃ¡ OK.</span>
                              </div>
                              {showSelfCheckPanel && (
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px]">
                                  <div className="rounded-lg border border-white/10 bg-black/30 px-2.5 py-2">
                                      <p className="text-gray-400">MicrÃ³fono</p>
                                  ...
                                  <div className="rounded-lg border border-white/10 bg-black/30 px-2.5 py-2">
                                      ...
                                      <p className={`font-bold ${(healthSnapshot.savedScriptsCount > 0 || healthSnapshot.storyScenesCount > 0) ? 'text-emerald-300' : 'text-amber-300'}`}>{healthSnapshot.savedScriptsCount} guardados Â· {healthSnapshot.storyScenesCount} escenas</p>
                                  </div>
                                  <div className="rounded-lg border border-white/10 bg-black/30 px-2.5 py-2">
                                      <p className="text-gray-400">Estado sesiÃ³n</p>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                              {[
                                  ...
                                  { id: 'shadow', label: 'Shadowing', sub: 'PronunciaciÃ³n', icon: 'audio-lines', go: () => setActiveTab('shadowing') },
                                  ...
                                  { id: 'progreso', label: 'Progreso', sub: 'Plan del dÃ­a y estadÃ­sticas', icon: 'bar-chart', go: () => setActiveTab('progreso') },
                          ...
                          <div className="mt-8 flex flex-wrap gap-3">
                              <button type="button" onClick={() => { setShowMullerHub(true); setMullerHubTab('voices'); }} className="px-4 py-2.5 rounded-xl bg-sky-700 hover:bg-sky-600 font-bold text-sm border border-sky-500/40 shadow-lg">Centro MÃ¼ller (voces Â· temas Â· IA Chrome)</button>
                  ...
                  {activeTab === 'comunidad' && !practiceActive && (
                      <div className="flex-1 flex flex-col overflow-y-auto hide-scrollbar p-4 md:p-8 max-w-4xl mx-auto w-full animate-in fade-in duration-500">
                          ...
                          <p className="text-[11px] text-gray-500 mb-4">Cuenta y ajustes se gestionan ahora desde el menÃº de usuario (arriba derecha) o el botÃ³n flotante de ajustes.</p>

                          {communitySubTab === 'cuenta' && (
                              <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-5 md:p-6 shadow-xl">
                                  {unifiedAuth ? (
                                      <div className="space-y-4">
                                          <p className="text-white font-bold text-lg flex items-center gap-2"><Icon name="check-circle" className="w-5 h-5 text-emerald-400" /> SesiÃ³n iniciada</p>
                                          <p className="text-[11px] font-bold uppercase tracking-wider text-violet-400">{unifiedAuth.source === 'supabase' ? 'Cuenta Supabase (nube Â· gratis)' : 'Cuenta solo en este navegador'}</p>
                                          ...
                                          <div className="rounded-xl border border-white/10 bg-black/25 p-3 space-y-2">
                                              ...
                                              <div className="flex flex-col sm:flex-row gap-2">
                                                  ...
                                                  >
                                                      {profileNameBusy ? 'Guardandoâ€¦' : 'Guardar nombre'}
                                                  </button>
                                              </div>
                                              {profileNameMsg ? <p className="text-xs text-gray-400">{profileNameMsg}</p> : null}
                                          </div>
                                      ...
                                      <div className="space-y-4">
                                          ...
                                          <label className="block text-xs font-bold text-gray-500 uppercase">ContraseÃ±a (mÃ­n. 6)</label>
                                          ...
                                          <button
                                              ...
                                              onClick={async () => {
                                                  ...
                                                  const errMap = {
                                                      ...
                                                      EMAIL_INVALID: 'Introduce un email vÃ¡lido.',
                                                      PASS_SHORT: 'La contraseÃ±a debe tener al menos 6 caracteres.',
                                                      EMAIL_TAKEN: 'Ese email ya estÃ¡ registrado en este dispositivo.',
                                                      BAD_CREDENTIALS: 'Email o contraseÃ±a incorrectos.',
                                                  };
                                                  try {
                                                      const client = mullerGetSupabaseClient();
                                                      if (client) {
                                          ...
                                          >
                                              {authBusy ? 'â€¦' : authMode === 'register' ? 'Crear cuenta' : 'Entrar'}
                                          </button>
                                          <p className="text-[11px] text-gray-500 leading-relaxed">
                                              {mullerGetSupabaseClient()
                                                  ? 'Con Supabase la contraseÃ±a va por Auth seguro de Supabase (gratis). Sin URL/key en index.html se usa cuenta local con PBKDF2 en el dispositivo.'
                                                  : 'Sin Supabase configurado: la contraseÃ±a se procesa con PBKDF2 solo en tu dispositivo; no hay servidor.'}
                                          </p>
                                      </div>
                                  )}
                              </div>
                          ...
                          {communitySubTab === 'economia' && (
                              <div className="space-y-6">
                                  <div className="rounded-2xl border border-amber-500/25 bg-slate-900/80 p-5">
                                      <h2 className="text-lg font-black text-white mb-2 flex items-center gap-2"><Icon name="coins" className="w-5 h-5 text-amber-400" /> EconomÃ­a de monedas</h2>
                                      <p className="text-xs text-gray-500 mb-3">
                                          {mullerGetSupabaseClient()
                                              ? 'Modo seguro: bonus/gastos pasan por RPC en Supabase con lÃ­mites diarios y cooldown. Evita monedas infinitas por trucos del cliente.'
                                      ...
                                      {walletLoading ? <p className="text-xs text-gray-500 mt-1">Sincronizando walletâ€¦</p> : null}
                                  ...
                                  <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-5">
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                          ...
                                          >
                                              ...
                                              <p className="text-xs text-gray-400 mt-1">1 vez al dÃ­a por usuario.</p>
                                          </button>
                                          <button
                                              type="button"
                                          ...
                                          >
                                              ...
                                              <p className="text-xs text-gray-400 mt-1">Abre tu enlace monetizado en nueva pestaÃ±a.</p>
                                          </button>
                                          <button
                                              type="button"
                                          ...
                                          >
                                              ...
                                              <p className="text-xs text-gray-400 mt-1">MÃ¡x 6/dÃ­a y cooldown de 15 min (backend).</p>
                                          </button>
                                          <div className="rounded-xl border border-white/10 bg-black/25 p-3">
                                              <p className="text-sm font-black text-white">Gana jugando</p>
                                              <p className="text-xs text-gray-400 mt-1">Racha diaria, sesiones completas y prÃ¡ctica oral ya suman. PrÃ³ximo paso: migrar TODAS las recompensas a RPC para blindaje total.</p>
                                          </div>
                                      </div>
                                  </div>
                                  ...
                                  <div className="rounded-2xl border border-fuchsia-500/30 bg-slate-900/80 p-5">
                                      ...
                                      <p className="text-xs text-gray-500 mb-3">MonetizaciÃ³n simple: botÃ³n de pago externo + estado premium en Supabase. Para activaciÃ³n automÃ¡tica real necesitarÃ¡s webhook (paso siguiente).</p>
                                      ...
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                          ...
                                          >
                                              ...
                                              <p className="text-xs text-gray-400 mt-1">Abre tu pÃ¡gina de cobro (Stripe/PayPal/etc).</p>
                                          </button>
                                          <button
                                              type="button"
                                              ...
                                              onClick={async () => {
                                                  const client = mullerGetSupabaseClient();
                                                  if (!client || !supabaseUser) { setEconomyMsg('Necesitas sesiÃ³n Supabase.'); return; }
                                                  ...
                                                  setEconomyMsg('Premium activado 30 dÃ­as (modo manual de pruebas).');
                                                  setAuthTick((x) => x + 1);
                                              }}
                                          >
                                              <p className="text-sm font-black text-violet-300">Ya paguÃ© (activar 30d)</p>
                                              <p className="text-xs text-gray-400 mt-1">BotÃ³n temporal para test hasta conectar webhook.</p>
                                          </button>
                                      </div>
                                  </div>
                              </div>
                              ...
                              <div className="space-y-6">
                                  {mullerSupabaseConfigured() ? (
                                      <div className="rounded-2xl border border-emerald-500/25 bg-slate-900/80 p-5 overflow-x-auto">
                                          ...
                                          {remoteProfiles === null ? (
                                              ...
                                              <p className="text-sm text-gray-500">AÃºn no hay filas en <code className="text-emerald-400/90">profiles</code>. Ejecuta el SQL del proyecto y registra un usuario.</p>
                                          ) : (
                                  ...
                                  <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-5 overflow-x-auto">
                                      ...
                                      {directoryLocals.length === 0 ? (
                                          <p className="text-sm text-gray-500">AÃºn no hay registros. Crea una cuenta en la pestaÃ±a Â«CuentaÂ».</p>
                                      ) : (
                                          <table className="w-full text-sm text-left">
                                  ...
                                  <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-5 overflow-x-auto">
                                      <h2 className="text-lg font-black text-white mb-3 flex items-center gap-2"><Icon name="bot" className="w-5 h-5 text-fuchsia-400" /> Bots de prÃ¡ctica (liga)</h2>
                                      <p className="text-xs text-gray-500 mb-3">Competidores simulados con puntuaciÃ³n semanal. No son usuarios reales.</p>
                                      <table className="w-full text-sm text-left">
                                          ...
                                          <tbody>
                                              {MULLER_BOT_PLAYERS.map((b) => (
                                                  <tr key={b.id} className="border-b border-white/5 text-gray-300">
                                                      ...
                                                      <td className="py-2 pr-2">{b.tag} Â· {b.lvl}</td>
                          ...
                          {communitySubTab === 'ligas' && (
                              <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-5 md:p-6 overflow-x-auto shadow-xl">
                                  <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                                      <h2 className="text-lg font-black text-white flex items-center gap-2"><Icon name="medal" className="w-6 h-6 text-amber-400" /> Liga semanal MÃ¼ller</h2>
                                      <span className="text-xs font-mono text-gray-500">Semana (lun): {leagueBoard.week}</span>
                                  </div>
                                  <p className="text-xs text-gray-500 mb-4">
                                      {mullerSupabaseConfigured()
                                          ? 'PuntuaciÃ³n global: se sube a Supabase (tabla league_scores) mientras practicas. Los bots son simulados y se mezclan en la tabla.'
                                          : 'Tu puntuaciÃ³n usa XP, monedas, racha, dictados y prÃ¡ctica oral. Los bots tienen puntuaciÃ³n simulada por semana (cambia cada lunes).'}
                                  </p>
                                          ...
                                          {leagueBoard.rows.map((r) => (
                                              <tr key={r.id} className={`border-b border-white/5 ${r.isSelf ? 'bg-violet-900/25' : ''}`}>
                                                  ...
                                                  <td className="py-2.5 pr-2">
                                                      ...
                                                      {r.isSelf ? <span className="ml-2 text-[10px] font-black uppercase px-1.5 py-0.5 rounded bg-violet-800/60 text-violet-200">TÃº</span> : null}
                                                  </td>
                  ...
                  {/* HISTORIA (Main Engine) con botÃ³n Ruido */}
                     ...
                     <div className="flex-1 flex flex-col relative w-full min-h-full justify-center items-center p-3 md:p-8">
                        <div className="absolute top-2 left-2 z-[12] flex flex-col gap-2 items-start max-w-[min(100%,min(420px,96vw))]">
                            <div className="flex flex-wrap gap-1 items-center">
                                ...
                                <ExerciseHelpBtn helpId="historia_herramientas" compact className="!text-amber-200/95 !border-amber-500/25" title="Barra de herramientas (Diktat, huecos, artÃ­culosâ€¦)" />
                            </div>
                            <div className="flex flex-wrap items-center gap-2 bg-black/50 border border-white/10 rounded-xl px-2 py-1.5">
                                ...
                                >
                                    ...
                                    {savedScripts.map((s) => (
                                        <option key={String(s.id)} value={String(s.id)}>{s.title || 'Sin tÃ­tulo'}</option>
                                    ))}
                        ...
                        {mode !== 'quiz' && mode !== 'interview' && !podcastMode && (
                            <div className="absolute top-2 right-2 flex flex-wrap items-center gap-1 md:gap-2 bg-black/60 p-1 rounded-xl border border-white/10 backdrop-blur-md z-10 max-w-[95%] justify-end">
                                ...
                                <button onClick={() => {setBlindMode(!blindMode); setDiktatMode(false); setPuzzleMode(false);}} className={`flex items-center gap-0.5 md:gap-1 px-2 md:px-3 py-1 rounded-lg text-[10px] md:text-xs font-bold transition ${blindMode ? 'bg-blue-400 text-black' : 'text-gray-300 hover:bg-white/10'}`}>{blindMode ? <Icon name="eye-off" className="w-3 h-3 md:w-4 md:h-4" /> : <Icon name="eye" className="w-3 h-3 md:w-4 md:h-4" />} OÃ­do</button>
                        ...
                        {mode === 'interview' ? (
                            ...
                            <div className="max-w-4xl w-full flex flex-col items-center justify-center animate-in zoom-in duration-500 p-4">
                                <div className="bg-emerald-900 text-emerald-200 border border-emerald-500 px-3 md:px-6 py-1 md:py-2 rounded-full font-bold flex items-center gap-2 mb-4 md:mb-6 text-xs md:text-sm"><Icon name="message-square" className="w-4 h-4 md:w-5 md:h-5" /> MÃ¼ndliche PrÃ¼fung Teil 2 Â· {oralQIdx + 1}/{MULLER_ORAL_B1_QUESTIONS.length}</div>
                                ...
                                <h2 className="text-base md:text-2xl text-gray-300 font-bold mb-2 uppercase tracking-widest text-center">PrÃ¼fer/in fragt:</h2>
                                ...
                                <div className="flex flex-col items-center gap-3 md:gap-4 w-full bg-black/40 p-4 md:p-8 rounded-2xl border border-white/10 shadow-2xl">
                                    ...
                                    <p className="text-gray-400 font-bold mt-1 text-center text-xs md:text-sm">MantÃ©n pulsado para responder (se compara con una frase modelo B1)</p>
                                    {spokenText && (
                            ...
                            <div className="max-w-4xl w-full flex flex-col items-center justify-center gap-4 md:gap-6 animate-in zoom-in duration-300 p-4">
                                ...
                                {!showCurrentTranslation ? (
                                    <button onClick={() => setShowCurrentTranslation(true)} className="text-gray-500 hover:text-white transition text-xs md:text-sm font-bold flex items-center gap-1 mt-2 border border-gray-700 rounded-full px-2 md:px-3 py-0.5 md:py-1"><Icon name="eye" className="w-3 h-3 md:w-4 md:h-4" /> Ver TraducciÃ³n</button>
                                ) : (
                                    ...
                                    <div className="flex flex-col items-center gap-4 md:gap-8 w-full mt-2 md:mt-4">
                                        ...
                                        <div className="flex gap-3 md:gap-4 w-full md:w-auto justify-center">
                                            ...
                                            <button onClick={() => runSingleSubmitAction('puzzle-check', () => {
                                                ...
                                                const norm = (s) => String(s || '').toLowerCase().replace(/[.,!?;:â€¦]/g, ' ').replace(/\s+/g, ' ').trim();
                            ...
                            <div className="max-w-3xl w-full flex flex-col items-center justify-center flex-1 py-8 md:py-16 px-4 gap-8 md:gap-10 animate-in fade-in">
                                <p className="text-violet-300 font-black text-xs uppercase tracking-[0.2em]">Solo audio Â· manos libres</p>
                                ...
                                <p className="text-gray-500 text-sm font-mono">Szene {sceneIndex + 1} / {guionData.length} Â· {activeScriptTitle}</p>
                            ...
                            <div className="max-w-4xl w-full flex flex-col gap-3 md:gap-6 animate-in fade-in relative mt-4 md:mt-8 p-3 md:p-0">
                                ...
                                {!showCurrentTranslation ? (
                                    <button onClick={() => setShowCurrentTranslation(true)} className="text-gray-500 hover:text-white transition text-[10px] md:text-sm font-bold flex items-center gap-1 w-max border border-gray-700 rounded-full px-2 md:px-3 py-0.5 md:py-1 mt-1 md:mt-2"><Icon name="eye" className="w-3 h-3 md:w-4 md:h-4" /> Ver TraducciÃ³n</button>
                                ) : (
                                ...
                                {!isPlaying && !podcastMode && (
                                    <div className="absolute right-0 top-0 md:-right-16 md:top-1/2 md:-translate-y-1/2 flex flex-col gap-2 md:gap-4">
                                        ...
                                        <button onClick={trySaveGrammarStructure} className="bg-cyan-600 text-white p-1.5 md:p-3 rounded-full border border-cyan-400 shadow-lg flex items-center justify-center group" title="Guardar GramÃ¡tica">
                                ...
                                {showGrammarPrompt && (
                                    <div className="absolute inset-0 z-50 bg-black/90 p-4 flex flex-col justify-center items-center rounded-2xl animate-in zoom-in">
                                        <div className="bg-cyan-950 border-2 border-cyan-500 p-4 md:p-8 rounded-2xl max-w-lg w-full shadow-[0_0_30px_rgba(6,182,212,0.4)]">
                                            ...
                                            <p className="text-cyan-100 mb-4 md:mb-6 text-xs md:text-sm">No he detectado ninguna regla automÃ¡tica en esta frase. Â¿QuÃ© gramÃ¡tica quieres guardar en tus Flashcards?</p>
              ...
              {activeTab === 'historia' && mode !== 'quiz' && mode !== 'interview' && !practiceActive && (
                  <div className="muller-historia-player-bar bg-gray-950 p-3 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-3 md:gap-4 border-t border-gray-800 z-20">
                      ...
                      <button type="button" onClick={exportScriptPDF} className="flex bg-red-700 hover:bg-red-600 border border-red-500 px-3 md:px-4 py-1.5 md:py-2 rounded-xl text-white font-bold items-center gap-2 shadow-lg transition text-xs md:text-sm shrink-0" title="PDF con alemÃ¡n, traducciÃ³n al espaÃ±ol, vocabulario y anÃ¡lisis"><Icon name="file-down" className="w-3 h-3 md:w-4 md:h-4" /> PDF Guion</button>
              ...
              {pdfStudyFullscreen && pdfStudyDoc ? (
                  <div className="fixed inset-0 z-[120] bg-black/95 flex flex-col">
                      <div className="flex items-center justify-between gap-3 px-3 md:px-5 py-3 border-b border-white/10 bg-slate-950/95">
                          <div className="min-w-0">
                              <p className="text-xs md:text-sm font-black text-cyan-100 truncate">PDF Coach Â· {pdfStudyDoc.name || 'Libro PDF'}</p>
                              <p className="text-[10px] md:text-xs text-cyan-300/80">PÃ¡gina {activePdfPageData.page || 1}/{pdfStudyDoc.totalPages || (pdfStudyDoc.pages || []).length || 1}</p>
                          </div>
                          <div className="flex items-center gap-2">
                              <button
                                  type="button"
                          ...
                          <section className="rounded-2xl border border-cyan-500/25 bg-black/35 p-3 md:p-4 space-y-3">
                              ...
                              {pdfStudyBlobUrl ? (
                                  ...
                                  <div className="rounded-xl border border-amber-600/35 bg-amber-950/35 p-3">
                                      <p className="text-xs text-amber-100">Vista PDF no disponible en esta sesiÃ³n. Vuelve a subir el PDF para ver el documento completo aquÃ­.</p>
                                  </div>
                              )}
                              <textarea
                                  value={activePdfPageData.text || ''}
                                  ...
                                  placeholder="Texto detectado de esta pÃ¡gina."
                              />
                          </section>
                          <section className="rounded-2xl border border-rose-500/25 bg-black/35 p-3 md:p-4 space-y-3">
                              <p className="text-xs font-black text-rose-200 uppercase tracking-wider">Notas de estudio por pÃ¡gina (stylus + teclado)</p>
                              <TabletWritingCanvas
        ...
        // ========== COMPONENTES FLOTANTES (SincronizaciÃ³n y Permiso MicrÃ³fono) ==========
        function FloatingButtons() {
            ...
            const requestMicPermission = async () => {
                try {
                    ...
                    if (!ok) return;
                    window.__mullerToast && window.__mullerToast('MicrÃ³fono concedido. Ya puedes usar reconocimiento de voz.', 'success');
                } catch (err) {
                    window.__mullerToast && window.__mullerToast('No se pudo obtener permiso del micrÃ³fono.', 'error');
                }
            };
            ...
            const exportData = React.useCallback(() => {
                ...
                window.__mullerToast && window.__mullerToast('Backup total exportado. GuÃ¡rdalo para sincronizar.', 'success');
            }, []);
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (data.type === 'muller_partial_v1' && data.part === 'srs') {
                            ...
                            window.__mullerToast && window.__mullerToast('SRS importado. Recargandoâ€¦', 'success');
                        ...
                        if (data.type === 'muller_partial_v1' && data.part === 'decks') {
                            ...
                            window.__mullerToast && window.__mullerToast('Mazos importados. Recargandoâ€¦', 'success');
                            window.location.reload();
                            return;
                        }
                        ...
                        window.__mullerToast && window.__mullerToast('Datos importados correctamente. Recargandoâ€¦', 'success');
                        window.location.reload();
                    } catch(err) { window.__mullerToast && window.__mullerToast('Archivo invÃ¡lido.', 'error'); }
                };
            ...
            const exportDecksOnly = () => {
                try {
                    ...
                    a.click();
                    URL.revokeObjectURL(url);
                    window.__mullerToast && window.__mullerToast('Exportados mazos (difÃ­cil/normal/gramÃ¡tica).', 'success');
            ...
            const showSyncHelp = () => {
                alert(
                    ...
                    "Esto copia TODO el estado guardado de la aplicaciÃ³n."
                );
            };
